...
<script runat="server">        
    ...
    private string sendRTXMass(string receivers,string type) {
        ...
        if (type == "wx")
            rtxResult = string.Format(rtxResult, "success", "提交发送微信提醒请求成功！");
        else if(type =="rtx")
            rtxResult = string.Format(rtxResult, "success", "提交发送BQQ提醒请求，成功：" + sucNums.ToString() + "个 失败：" + errNums + "个。");
        else if(type == "all")
            rtxResult = string.Format(rtxResult, "success", "提交发送BQQ提醒请求，成功：" + sucNums.ToString() + "个 失败：" + errNums + "个，并且已发送微信通知！");
    ...
    private string sendRTXmessage(string dataid,string type)
    {
        ...
        {            
            string sql = @" select max(name) name,cname into #wxxx from wx_t_customers group by cname;
                            ...
                            left join wx_t_AppAuthorized wa on wa.systemid=5 and wa.systemkey=a.sjrzh
                            left join wx_t_customers wc on wc.id=wa.userid
            ...
            if (errInfo == "")
            {
                if (dt.Rows.Count > 0)
                {
                    ...
                    string sjr = dt.Rows[0]["sjr"].ToString();
                    string wyReceiver = dt.Rows[0]["wyxm"].ToString();
                    //20150922 liqf当关联不到用户的RTX信息时直接尝试将消息发给收件人，避免数据不同步的情况
                    string blReceiver = dt.Rows[0]["rtxname"].ToString() == "" ? sjr : dt.Rows[0]["rtxname"].ToString();
                    string WXwyname = dt.Rows[0]["wywxname"].ToString();
                    string WXblname = dt.Rows[0]["sjrwxname"].ToString();

                    string kddh = dt.Rows[0]["kddh"].ToString();//快递单号
                    string kdgs=dt.Rows[0]["kdgs"].ToString();//快递公司
                    string jbr = dt.Rows[0]["jbr"].ToString();//经办人
                    
                    if (wyReceiver != "")
                    {
                        tmptxt = "的领导【" + sjr + "】";
                        leaderMsg = string.Format(leaderMsg, kddh, kdgs, jbr, wyReceiver);                                   
                    }
                    msg = string.Format(msg, kddh, kdgs, jbr, tmptxt); 
                    ...
                    {                        
                        using (RTXHelper rtxHelper = new RTXHelper())
                        {
                            if (wyReceiver == "")
                            {
                                ...
                                if (type == "rtx")
                                {
                                    errInfo = rtxHelper.SendRTXMSG(blReceiver, title, msg);
                                }
                                else if (type == "wx")
                                {
                                    wxInfo = sendWXnotice(WXblname, msg);
                                }
                                else if (type == "all")
                                {
                                    errInfo = rtxHelper.SendRTXMSG(blReceiver, title, msg);
                                ...
                                if (type == "rtx")
                                {                                    
                                    errInfo = rtxHelper.SendRTXMSG(blReceiver, title, leaderMsg);
                                    errInfo = rtxHelper.SendRTXMSG(wyReceiver, title, msg);
                                }
                                else if (type == "wx")
                                {
                                    wxInfo = sendWXnotice(WXblname, leaderMsg);
                                    wxInfo = sendWXnotice(wyReceiver, msg);
                                }
                                else if (type == "all")
                                {
                                    errInfo = rtxHelper.SendRTXMSG(blReceiver, title, leaderMsg);
                                    errInfo = rtxHelper.SendRTXMSG(wyReceiver, title, msg);
                                    wxInfo = sendWXnotice(WXblname, leaderMsg);
                                    wxInfo = sendWXnotice(WXwyname, msg);
                                }
                            }
                            ...
                            switch (type) { 
                                case "rtx":
                                    rtxResult = updateCount(type, errInfo, dataid);
                                    break;
                                case "wx":
                                    rtxResult = updateCount(type, wxInfo, dataid);
                                    break;
                                case "all":
                                    rtxResult = updateCount(type, "", dataid);
                                    break;
                            }
                    ...
                    rtxResult = string.Format(rtxResult, "error", "发送BQQ、微信提醒失败:查询不到相关记录！");
                }                    
            }
            else
                rtxResult = string.Format(rtxResult, "error", "发送BQQ、微信提醒失败:" + errInfo);
        }

        return rtxResult;
    }

    public string updateCount(string type,string info,string id) {
        string rtxResult = @"{{""type"":""{0}"",""msg"":""{1}""}}";
        try
        {
            using (LiLanzDALForXLM dal = new LiLanzDALForXLM())
            {
                ...
                if (type == "rtx")
                    ...
                    sql = " update t_kdlqdjb set wxNoticeNo=wxNoticeNo+1 where id=@id;";
                else if (type == "all")
                {
                    sql = "update t_kdlqdjb set rtxno=rtxno+1 where id=@id;update t_kdlqdjb set wxNoticeNo=wxNoticeNo+1 where id=@id;";
                }
            ...
            rtxResult = string.Format(rtxResult, "error", "提交发送BQQ、微信提醒请求失败！");
        }
        
        rtxResult = string.Format(rtxResult, "success", "提交发送BQQ、微信提醒请求成功！");                  
        return rtxResult;
    }
    
    //调用发送微信消息的接口
    public string sendWXnotice(string receiver,string msg) {
        string rtMsg = "";
        if (receiver != "") {
            receiver = receiver.Trim();
            MsgClient msgclient = new nrWebClass.MsgClient("192.168.35.63", 21000);
            rtMsg = msgclient.ExpressMsgSend(receiver, msg);
        }
