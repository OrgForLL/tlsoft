...
<script runat="server">
        ...
        public static List<zbInfo> GetAllInfo(string JsonStr)
        {
            ...
            for (int i = 0; i < testItem.Count; i++)
            {
                ...
                else
                {
                    ...
                    zbinfo = new zbInfo();
                    zbinfo.委托序号 = testItem[i].GetJsonValue("OrderNo");
                    zbinfo.报告编号 = paths[paths.Length - 1].Split('.')[0];
                    zbinfo.样品名称 = testItem[i].GetJsonValue("SampleName");
                    zbinfo.样品货款号 = sphh;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            date = DateTime.Parse(Request["date"].ToString()).ToString("yyyy-MM-dd");
        }
        ...
        {
            string ids = GetTBSJIDS(date, bs);
            //下载pdf到本地 start
            string downPDFInfo = SynchronousDate.tbData("downPDF", userid, "自动同步", "", ids);
    ...
    public string GetTBSJIDS(string tbrq, string bs)
    {
        string sql = @"
            SELECT id 
            FROM yf_t_syjcbg  
            WHERE CONVERT(VARCHAR(50),czrq,112)=CONVERT(VARCHAR(50),CAST('{0}' AS DATETIME),112) and bs='{1}'
            ORDER BY id DESC
        ";
    ...
    public DataTable GetTBJL(string tbrq, string bs)
    {
        string sql = @"
            ...
            WHERE CONVERT(VARCHAR(50),a.czrq,112)=CONVERT(VARCHAR(50),CAST('{0}' AS DATETIME),112) and bs='{1}'
            ORDER BY a.id DESC
        ";
        sql = string.Format(sql, tbrq, bs);
