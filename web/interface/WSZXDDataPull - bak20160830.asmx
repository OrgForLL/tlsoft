...
public class WSZXDDataPull : System.Web.Services.WebService
{
    ...
    //设置材料的仓位信息
    [WebMethod]
    public string setCLInfo(DataTable cwInfo,string userName, string tzid, string qwtm, string djid, string djly, string ids)
    {
        ...
        using (LiLanzDALForXLM dal = new LiLanzDALForXLM())
        {
            if(cwInfo!=null)
            {
                ...
                string str_sql = " DECLARE @qwid INT,@lydjid VARCHAR(50),@djly varchar(50); select @qwid=a.id,@lydjid='" + djid + "' from WarehouseLoc a where a.tzid='" + tzid + "' and a.dm='" + qwtm + "'; ";
                for (int i = 0; i < dr.Length;i++)
                {
                    //设置多个时
                    str_sql += " if('" + djly + "'='S') ";
                    str_sql += "    begin ";
                    str_sql += "      SELECT @djly=SUBSTRING(isnull(ids,0),1,1) FROM  cl_t_cwdjtmIDgl WHERE id='" + djid + "' ";
                    str_sql += "      if(@djly='1') ";
                    str_sql += "         select @lydjid=a.id from cl_v_dddjmx a where a.djlx='" +lydjlx + "' AND a.mxid='"+ dr[i]["mxid"] +"' ";
                    str_sql += "    end ";                    
                    
                    //当djly=1时获取单据id
                    str_sql += " if('" + djly + "'='1') ";
                    str_sql += "    begin ";
                    str_sql += "      SELECT @lydjid=a.id ";
                    str_sql += "      FROM CL_T_dddjb a  ";
                    str_sql += "      where (CAST(DATENAME(yyyy,a.zdrq) AS VARCHAR(max))+CAST(DATENAME(mm,a.zdrq) AS VARCHAR(max))+CAST(a.djh AS VARCHAR(max)))='" + djid + "' AND a.djlx=624 ";
                    str_sql += "    end ";
                    
                    str_sql += " IF NOT EXISTS(SELECT * FROM sc_t_chqwwhb WHERE lydjid=@lydjid AND lydjlx='" +lydjlx + "' AND lydjmxid='"+ dr[i]["mxid"] +"') ";
                    str_sql += "    INSERT INTO sc_t_chqwwhb (gzh,chdm,ckqwid,sl,lydjid,lydjlx,lydjmxid,createDate,Creater) VALUES('" + dr[i]["scddbh"] + "','" + dr[i]["chdm"] + "',@qwid,'" + dr[i]["sl"] + "',@lydjid,'" + lydjlx + "','"+ dr[i]["mxid"] +"',GETDATE(),'" + userName + "'); ";
                    str_sql += " else ";
                    str_sql += "    UPDATE sc_t_chqwwhb SET gzh='" + dr[i]["scddbh"] + "',chdm='" + dr[i]["chdm"] + "',ckqwid=@qwid,sl='" + dr[i]["sl"] + "' WHERE lydjlx='" +lydjlx + "' AND lydjmxid='"+ dr[i]["mxid"] +"'  ";
                }                
    ...
    //获取要维护仓位的材料信息
    [WebMethod]
    public string getCLInfo(string djid, string djly, string tzid)
    {
        string rtMsg = "", errInfo = "";
        using (LiLanzDALForXLM dal = new LiLanzDALForXLM())
        {
            ...
            else
            {
                ...
                if(djly=="1" || djly=="S")
                {
                    if(djly=="S")
                    {
                        str_sql = @" declare @djids varchar(500) 
                                   ...
                                   select distinct a.scddbh,a.chdm,a.sl,a.mxid,a.ckid,xx.id cwid  
                                   from #myzb a 
                                   inner join cl_v_chdmb c on a.chdm=c.chdm 
                                   inner join  cl_T_chgzhkcmx ch on a.chdm=ch.chdm AND a.scddbh=ch.gzh AND a.ckid=ch.ckid AND isnull(ch.sl,0)<>0  
                ...
                {
                    str_sql = @" SELECT DISTINCT a.scddbh,a.chdm,a.sl,a.mxid,a.ckid,xx.id cwid  
                              FROM    cl_v_kcdjmx a
