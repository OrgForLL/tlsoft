...
<script runat="server">        
    ...
    private string sendRTXMass(string receivers) {
        ...
        if (RCS.Length > 0) {
            for (int i = 0; i < RCS.Length; i++) {
                ...
                string errInfo = sendRTXmessage(RCS[i]);
    ...
    //发送RTX消息提醒
    private string sendRTXmessage(string dataid)
    {
        string rtxResult = @"{{""type"":""{0}"",""msg"":""{1}""}}";
        string title = "快递通知";
        string msg = "您好，您有一份快递已到公司，信息如下：\n【快递单号】：{0}\n【快递公司】：{1}\n【经办人】：{2}\n请及时带上工作牌到前台或找相关负责人领取！\n" + DateTime.Now.ToString("yyyy年M月d日 HH:mm");
        string errInfo = "";
        using (LiLanzDALForXLM dal = new LiLanzDALForXLM())
        {
            string sql = " select a.sjr,a.kddh,kd.kdgs,a.jbr,isnull(rtx.rtxname,'') rtxname from t_kdlqdjb a left join t_kdgsdmb kd on a.kdgsdm=kd.dm left join t_rtxuser rtx on rtx.username=a.sjr where a.id=@id;";
            DataTable dt = null;
            ...
            if (errInfo == "")
            {
                if (dt.Rows.Count > 0)
                {
                    msg = string.Format(msg, dt.Rows[0]["kddh"].ToString(), dt.Rows[0]["kdgs"].ToString(),dt.Rows[0]["jbr"].ToString());
                    string receiver = clsNetExecute.GetEncodeValue(dt.Rows[0]["rtxname"].ToString(), System.Text.Encoding.GetEncoding("GB2312"));
