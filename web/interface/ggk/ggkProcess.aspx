...
<script runat="server">
    ...
    private const double WINPR = 0.7;//游戏中奖因子
    ...
    private void GetMyGift(string userid, string filter) {
        ...
        using (LiLanzDALForXLM dal = new LiLanzDALForXLM(connStr))
        {
            string str_sql = @"select a.id,g.gamename,a.gameid,a.gametoken,a.userid,a.prizeid,b.prizename,b.prizedesc,a.isget,a.isactive,
                                ...
                                case when a.validtime<getdate() and a.prizeid in (1,2) then 0 else 1 end isvalid,a.validtime
...
//                                and a.activetime>=convert(varchar(10),@gtime,120) 
//                                and a.activetime<dateadd(day,1,convert(varchar(10),@gtime,120))
//                                group by a.prizeid;
...
//                                drop table #tmp;";
            ...
            //改为配置表设多少就新增多少进去
            string str_sql = @" select a.*,a.numsperday todaynums from tm_t_prize a where a.id<>0;";
    ...
    private void GetGameData(string gameid, string userid)
    {
        ...
        using (LiLanzDALForXLM dal = new LiLanzDALForXLM(connStr))
        {
            ...
            if (errinfo == "")
            {
                ...
                {
                    //接下来先判断用户用户的中奖情况 
                    //20151216 09:00 官部、小姚确认中奖概率调整为0.2，纪念奖还可以再中 <>4 ==4
    ...
    private void ConsumeGameToken(string token, string userid, string gameid)
    {
        using (LiLanzDALForXLM dal = new LiLanzDALForXLM(connStr))
        {
            string str_sql = @" if not exists(select top 1 1 from tm_t_gamerecords where gametoken=@token and userid=@userid)
                                ...
                                begin
                                declare @gid int;declare @uid int;declare @pid int;declare @isactive int;declare @lpdj int;
                                declare @djid int;declare @djxm varchar(50);declare @djdh varchar(20);declare @djsfz varchar(50);
                                select @djid=a.id,@djxm=a.dpxm,@djdh=a.dh,@djsfz=a.sfz
                                from (
                                select top 1 lp.id,lp.dpxm,lp.dh,lp.sfz
                                ...
                                select @uid,isnull(@djid,0),isnull(@djxm,''),isnull(@djdh,''),isnull(@djsfz,'');
                                end";
            List<SqlParameter> paras = new List<SqlParameter>();
                ...
                if (dt.Rows.Count > 0)
                {
                    if (dt.Rows[0][0].ToString() == "00")
                        ...
                        if (dt.Rows[0][1].ToString() == "0")
                        {
                            ...
                            clsSharedHelper.WriteSuccessedInfo("您在领票时已经登记过信息|" + dt.Rows[0][2].ToString().Trim() + "|" + dt.Rows[0][3].ToString().Trim() + "|" + dt.Rows[0][4].ToString().Trim());
                        }
                    }
                    else
                        clsSharedHelper.WriteSuccessedInfo("");
    ...
    public void Prizer100() {
        using (LiLanzDALForXLM dal = new LiLanzDALForXLM(connStr)) {
            string str_sql = @" select top 100 substring(u.wxnick,1,1)+'**' wxnick,b.prizename,
                                substring(convert(varchar(20),a.createtime,120),12,5) createtime 
                                from tm_t_getprizerecords a 
                                inner join wx_t_vipbinging u on a.userid=u.id and u.objectid=101
                                inner join tm_t_prize b on a.prizeid=b.id
                                where a.gameid=1 and a.prizeid<>0 and a.prizeid<>4
