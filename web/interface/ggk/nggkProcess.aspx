            ...
            lock (lockPrize)
            {
                using (LiLanzDALForXLM dal = new LiLanzDALForXLM(connStr))
                {
                    string str_sql = @"select a.*,isnull(b.prizename,'') prizename 
                                       ...
                                       left join wx_t_prize b on a.prizeid=b.id and b.gameid=1 and b.sskey=7
    ...
    private void DelGameDatas(string GTime)
    {
        using (LiLanzDALForXLM dal = new LiLanzDALForXLM(connStr))
        {
            string str_sql = @"delete from tm_t_gamerecords 
                                ...
                                and gametime<dateadd(day,1,convert(varchar(10),@gtime,120)) and sskey=@sskey;";
        ...
        using (LiLanzDALForXLM dal = new LiLanzDALForXLM(connStr))
        {
            string str_sql = @" if not exists(select top 1 1 from wx_t_gamerecords where gametoken=@token and userid=@userid and sskey=@sskey and gameid=@gameid)
                                ...
                                begin
                                  ...
                                  if isnull(@pid,0)<>0
                                    insert into wx_t_getprizerecords(gameid,gametoken,prizeid,isget,operator,userid,createtime,isactive,activetime,sskey)
                                    values(@gameid,@token,@pid,0,'',@userid,getdate(),@isactive,getdate(),@sskey);
                                  update wx_t_usergameinfos set nowcounts=nowcounts+1 where userid=@userid and gameid=@gameid and sskey=@sskey;
                                  select '11';
                                end";
            List<SqlParameter> paras = new List<SqlParameter>();
