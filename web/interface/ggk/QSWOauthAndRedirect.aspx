...
<script runat="server">
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        string psharedkey = Convert.ToString(Request.Params["rand"]);
            ...
            if (clsWXHelper.CheckResult(ConfigKey,content))
            {
                ...
                using (clsJsonHelper jh = clsJsonHelper.CreateJsonHelper(content)) {
                    ...
                    if (clsWXHelper.CheckResult(ConfigKey, content))
                    {
                        ...
                        if (UID > 0) {
                            //clsSharedHelper.WriteSuccessedInfo(UID.ToString());
                            Session["QSW_UID"] = UID;
        ...
        {            
            using (LiLanzDALForXLM dal62 = new LiLanzDALForXLM(WXDBConStr)) {
                ...
                if (errinfo == "")
                    ...
                    writeLog("利郎轻商务游戏 检查创建用户游戏档案失败！错误：" + errinfo);
                    return -1;
                }
            }
        }//end try
        ...
        try
        {
            using (clsJsonHelper jh = clsJsonHelper.CreateJsonHelper(content))
            {
                ...
                using (LiLanzDALForXLM dal10 = new LiLanzDALForXLM(ConnStr))
                {
                    ...
                    {
                        writeLog("利郎轻商务游戏 检查创建用户信息档案失败！错误：" + strInfo);
                        return 0;
                    }
                }
            }
