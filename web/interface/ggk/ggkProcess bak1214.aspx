...
<script runat="server">
    ...
    private const double WINPR = 0.1;//游戏中奖因子
    ...
    private void GetGiftDetail(string token,string userid,string gameid) {
        using (LiLanzDALForXLM dal = new LiLanzDALForXLM(connStr))
        {
            string str_sql = @"select isnull(b.prizename,'') pname,isnull(b.prizedesc,'') pdesc,c.gamename,a.createtime gettime,a.validtime,
                                case when getdate()>a.validtime then '已失效' when a.isactive=0 and a.isget=0 then '未激活'
                                 when a.isactive=1 and a.isget=0 then '已激活可领取' else '已领取' end status
                                from tm_t_getprizerecords a
                    ...
                    {                                         
                        ...
                        {                            
                            ...
                            for (int j = 0; j < prizeNum; j++)
                            {
                                ...
                                //sb.AppendFormat("insert into tm_t_gamerecords(gameid,userid,gametoken,isconsume,prizeid,activetime,gametime) values (0,0,'"+ gametoken+ "',0,'"+prizeID+"','"+activetime+"','"+ GTime+ "');");
                            }//end for j                             
    ...
    private void GetGameData(string gameid, string userid)
    {
        ...
        using (LiLanzDALForXLM dal = new LiLanzDALForXLM(connStr))
        {
            ...
            if (errinfo == "")
            {
                ...
                {
                    //接下来先判断用户当天是否有存在有效期内的礼券  未最终确定
                    str_sql = @"select top 1 1 from tm_t_getprizerecords where userid=@userid";
                    /*
    ...
    private void ConsumeGameToken(string token, string userid, string gameid)
    {
        using (LiLanzDALForXLM dal = new LiLanzDALForXLM(connStr))
        {
            string str_sql = @" if not exists(select top 1 1 from tm_t_gamerecords where gametoken=@token and userid=@userid)
                                ...
                                begin
                                declare @gid int;declare @uid int;declare @pid int;
                                select @gid=a.id,@uid=isnull(b.id,0),@pid=a.prizeid
                                from tm_t_gamerecords a 
                                ...
                                if isnull(@pid,0)<>0
                                insert into tm_t_getprizerecords(gameid,gametoken,prizeid,isget,operator,userid,createtime)
                                values(@gameid,@token,@pid,0,'',@userid,getdate());
    ...
    private void RegisterUserInfo(string userid, string idcard, string phone, string uname)
    {
        using (LiLanzDALForXLM dal = new LiLanzDALForXLM(connStr))
        {
            string str_sql = @"if not exists(select top 1 1 from tm_t_userinfo where idcard=@idcard and phone=@phone)                             
                                begin
            ...
            if (errinfo == "" && dt.Rows.Count > 0)
            {
                ...
                if (result == "00")
                    clsSharedHelper.WriteErrorInfo("对不起，你输入的身份证和手机号已经被使用过！");
                else if (result == "11")
                    clsSharedHelper.WriteSuccessedInfo("");
            }
            else
