...
<script runat="server">
    ...
    public void staticWXUsers(string key, string khid, string wxprovince)
    {
        ...
        using (LiLanzDALForXLM dal = new LiLanzDALForXLM("server='192.168.35.10';uid=ABEASD14AD;pwd=+AuDkDew;database=tlsoft"))
        {
            ...
            {
                //str_sql = @"
                //         select count(openid) users from (
                //         --已关注VIP客户
                ...
                //         --后期新增了直接关注店铺的功能，将其也算进来
                //         union
                //         select wxopenid openid from wx_t_vipbinging where khid=@khid0) a";
                ...
                str_sql = @"select count(wxOpenid) users
                            ...
                            where kh.ccid+'-' like @khid";
                khid = "%-" + khid + "-%";
                List<SqlParameter> paras = new List<SqlParameter>();
                paras.Add(new SqlParameter("@khid", khid));
                //paras.Add(new SqlParameter("@khid0", khid0));
                object scalar;
        ...
        using (LiLanzDALForXLM dal = new LiLanzDALForXLM("server='192.168.35.10';uid=ABEASD14AD;pwd=+AuDkDew;database=tlsoft"))
        {
            ...
            else
            {
                str_sql = @" --已关注VIP客户
                         ...
                         --后期新增了关注店铺的功能，将其也算进来
                         union
                         select wxopenid openid from wx_t_vipbinging where khid=@khid0";
            }
