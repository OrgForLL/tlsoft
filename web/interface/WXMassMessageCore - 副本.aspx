...
<script runat="server">
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        switch (ctrl)
        {
            ...
            case "getOpenIDList":
                ...
                List<string> openids = getOpenIDList(khid, "");
            ...
            if (errinfo == "")
            {
                if (dt.Rows.Count > 0)
                {
                    ...
                    if (msgtype == "mpnews")
                    {
                        ...
                        clsSharedHelper.WriteInfo("【" + DateTime.Now.ToString() + "】 发送报告：总共发送【" + sendCounts.ToString() + "】次（每次8000个用户），其中成功【" + succesCount.ToString() + "】，失败【" + failCount.ToString() + "】");
                    }
                }
                else
    ...
    public void staticWXUsers(string key,string khid,string wxprovince) {
        ...
        {
            string str_sql = "";
            if (key == "5" && khid != "" && khid != "0")
            {
                str_sql = @" select sum(users) users 
                         from (
                          select count(wx.wxopenid) users
                          from yx_t_vipkh a                                    
                          inner join wx_t_vipbinging wx on a.id=wx.vipid
                          inner join yx_t_khb kh on a.khid=kh.khid and kh.ccid+'-' like @khid
                          where a.ty=0 and a.kh<>'' and wx.wxopenid<>''
                          --粉丝用户
                          union all
                          select count(wx.wxopenid) users
                          from rs_t_Rydwzl a 
                          inner join wx_t_OmniChannelUser o on o.relateid=a.id
                          inner join wx_t_vipsalerbind vs on vs.salerid=o.id
                          inner join wx_t_vipbinging wx on wx.wxopenid=vs.openid
                          inner join yx_t_khb kh on kh.khid=a.tzid and kh.ccid+'-' like @khid
                          where a.rzzk='1' and vs.vipid=0 and wx.wxopenid<>''
                         ) a";
                khid = "%-" + khid + "-%";
                List<SqlParameter> paras = new List<SqlParameter>();
                ...
                str_sql = "select count(wxopenid) users from wx_t_vipbinging where objectid=@objectid and wxprovince like @wxprovince;";
                //WriteLog(str_sql + "|" + objectid + "|" + wxprovince);
                List<SqlParameter> paras = new List<SqlParameter>();
        ...
        using (LiLanzDALForXLM dal = new LiLanzDALForXLM("server='192.168.35.62';uid=sa;pwd=ll=8727;database=weChatPromotion"))
        {
            ...
            if (khid == "-1")
            {
                str_sql = @"select 'oarMEt8bqjmZIAhImSXBAg0G7F0I' openid 
                            ...
                            ";
                //str_sql = "select top 8000 wxopenid openid from wx_t_vipbinging;";
            }else if (khid == "0" && wxprovince != "") {
                switch (key)
                {
                    case "5":
                        objectid = "1";
                ...
                wxprovince = "%" + wxprovince + "%";
                str_sql = "select wxopenid openid from wx_t_vipbinging where objectid=@objectid and wxprovince like @wxprovince;";
            }else{
                str_sql = @" --已关注VIP客户
                         select wx.wxopenid openid
                         from yx_t_vipkh a                                    
                         inner join wx_t_vipbinging wx on a.id=wx.vipid
                         inner join yx_t_khb kh on a.khid=kh.khid and kh.ccid+'-' like @khid
                         where a.ty=0 and a.kh<>'' and wx.wxopenid<>''
