...
<script runat="server"> 
        ...
        else
        {
            switch (ctrl)
            {
                case "syncData":
                    ...
                    break;
                case "syncData1":
                    bdate = Convert.ToString(Request.Params["bdate"]);
                    edate = Convert.ToString(Request.Params["edate"]);
                    if (bdate == "" || bdate == null || edate == "" || edate == null)
                        rtMsg = @"{""type"":""ERROR"",""msg"":""日期参数有误！""}";
                    else
                    {
                        ...
                        if (gz.IndexOf("WARN") > -1 && fz.IndexOf("WARN") > -1)
                        {
                            rtMsg = @"{""type"":""WARN"",""msg"":""检测数据不存在！！！""}";
                        }
                        else if ((gz.IndexOf("SUCCESS") > -1 || gz.IndexOf("WARN") > -1) && (fz.IndexOf("SUCCESS") > -1 || fz.IndexOf("WARN") > -1))
                        {
                            rtMsg = @"{""type"":""SUCCESS""}";
                        }
                        else
                        {
                            if (gz == "" && fz != "")
                            {
                                rtMsg = @"{""type"":""ERROR"",""msg"":""广州检测数据同步失败！！！""}";
                            }
                            else if (fz == "" && gz != "")
                            {
                                rtMsg = @"{""type"":""ERROR"",""msg"":""福州检测数据同步失败！！！""}";
                            }
                            else
                            {
    ...
    /// <returns></returns>
    public string pullDatas1(String startDate, String endDate)
    {
        String url = @"http://www.ffib.cn/query/55a81b96c60f8918e43.php?bdate={0}&edate={1}";
        String rtMsg = "",jls="0";
