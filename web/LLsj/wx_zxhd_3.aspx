...
<head id="Head2" runat="server">
    ...
    <script runat="server">
        ...
        {
            int hdid = Int32.Parse(Context.Request["id"]);
            // string strSQL = string.Format(@"select top 5 isnull(hdnr,''),isnull(tp,'') from t_zxhd  order by zdrq", new object[] { });
            IDataReader reader = (IDataReader)sqlHelp.MyDataRead(sqlHelp.GetConn(), "select  isnull(hdnr,''),isnull(tp,'') from t_zxhd where id="+hdid+" order by zdrq");
            while (reader.Read())
            {
                
                STRB.AppendFormat("<li >{0}<img class=img src=\"" + UpFilePath + "{1}\" ></li>", reader[0], reader[1]);
    ...
    <script language="javascript" type="text/javascript">
        function BUTTON2_onclick() {
            window.location.href = 'wx_zxhd_2.aspx';
            //history.go(-1);
        }
