 ...
 */
    ...
    public void LoadByBillNo()
    {
        ...
        string rt = clsNetExecute.HttpRequest(string.Format("{0}/LoadByBillNo?storeid={1}&billNo={2}", svrOrderUrl, storeid, billNo));
    ...
    public void LoadPayMode()
    {
        ...
        string rt = clsNetExecute.HttpRequest(string.Format(svrOrderUrl + "/LoadPayMode?orgid={0}&mode={1}", orgid, mode));
    ...
    public void GetSales()
    {
        ...
        string rt = clsNetExecute.HttpRequest(string.Format(svrOrderUrl + "/GetSales?storeid={0}", storeid));
        clsSharedHelper.WriteInfo(rt);
    }
    /// <summary>
    /// 打印保存
    ...
    public void SavePrint()
    {
        ...
        foreach (Dictionary<string, object> dPayMode in LPayMode)
        {
            if (Convert.ToInt32(dPayMode["payID"]) == -25)
            {
                ...
                if (point < Convert.ToInt32(dPayMode["exchangePoints"]))
                {
        ...
        //改变积分标识
        if (djson.ContainsKey("vipCardCode"))
        {
            if (!string.IsNullOrEmpty(Convert.ToString(djson["vipCardCode"])))
            {
                djson["pointsFlag"] = Convert.ToInt32(djson["pointsFlag"]) == 1 ? 0 : 1;
                jsondata = JsonConvert.SerializeObject(djson);
            }
        }

        string rt = clsNetExecute.HttpRequest(svrOrderUrl + "/SavePrint?data=" + jsondata);
        clsSharedHelper.WriteInfo(rt);
    ...
    {
        string storeid = Convert.ToString(HttpContext.Current.Request.Params["storeid"]);
        string rt = clsNetExecute.HttpRequest(svrOrderUrl + "/LoadOrderList?storeid=" + storeid);
        clsSharedHelper.WriteInfo(rt);
    }
    /// <summary>
    /// 读取单据列表(挂单列表)
    ...
    public void GetOrder()
    {
        ...
        string rt = clsNetExecute.HttpRequest(svrOrderUrl + "/GetOrder?billID=" + billID);
        clsSharedHelper.WriteInfo(rt);
    }
    /// <summary>
    /// (微信)卡券核销
    ...
    public void LoadSaler()
    {
        ...
        string IsClert = Convert.ToString(HttpContext.Current.Request.Params["IsClert"]);
        string rt = clsNetExecute.HttpRequest(string.Format(svrOrderUrl + "/LoadSaler?orgid={0}&cname={1}&IsClert={2}", orgid, cname, IsClert));
        clsSharedHelper.WriteInfo(rt);
    }
    /// <summary>
    /// 结算保存(保存为草稿)
    ...
    public void SaveOrder()
    {
        ...
        try
        {
            ...
            rt = clsNetExecute.HttpRequest(svrOrderUrl + "/OrdSave", "data=" + jsondata, "POST", "UTF-8", 10000);
        }
        catch (Exception ex)
        {
    ...
    public void LoadProduct()
    {
        ...
        string orgid = Convert.ToString(HttpContext.Current.Request.Params["orgid"]);
        string storeid = Convert.ToString(HttpContext.Current.Request.Params["storeid"]);
        string isCloudOrder = Convert.ToString(HttpContext.Current.Request.Params["isCloudOrder"]);
        if (string.IsNullOrEmpty(isCloudOrder))
        {
            isCloudOrder = "0";
        }
        //    clsSharedHelper.WriteInfo("2222");
         //string rt = clsNetExecute.HttpRequest(string.Format("{0}/LoadProduct?code={1}&orgid={2}&storeid={3}", xlmbaseurl, code, orgid, storeid));
         string rt = clsNetExecute.HttpRequest(string.Format("{0}/LoadProduct?code={1}&orgid={2}&storeid={3}&isCloudOrder={4}", policiesurl, code, orgid, storeid, isCloudOrder), "", "POST", "UTF-8", 10000);
        //  clsSharedHelper.WriteInfo(string.Format("{0}/LoadProduct?code={1}&orgid={2}&storeid={3}&isCloudOrder={4}", policiesurl, code, orgid, storeid,isCloudOrder));
        clsSharedHelper.WriteInfo(rt);
    }
    ...
    public void InitCurrent()
    {
        ...
        string rt = clsNetExecute.HttpRequest(string.Format("{0}/InitCurrent?storeid={1}&userid={2}", vipServerUrl, storeid, userid));
    ...
    public void IsAllowStore()
    {
        string storeid = Convert.ToString(HttpContext.Current.Request.Params["storeid"]);
        Dictionary<string, object> drt = new Dictionary<string, object>();
        if (string.IsNullOrEmpty(storeid))
        {
            res = ResponseModel.setRes(0, false, "确实必要参数");
            clsSharedHelper.WriteInfo(JsonConvert.SerializeObject(res));
        }

        using (LiLanzDALForXLM dal = new LiLanzDALForXLM(1))
        {
            string MySql = string.Format("SELECT  b.ccid FROM t_mdb a INNER JOIN dbo.yx_t_khb b ON a.khid=b.khid WHERE a.mdid={0}", storeid);
            Object obj;
            String errInfo = dal.ExecuteQueryFast(MySql, out obj);
            if (errInfo != "" || obj == null)
            {
                res = ResponseModel.setRes(201, "", errInfo);
            }
            ...
            {
                if (Convert.ToString(obj).IndexOf("-1900-") > 0)
                {
                    res = ResponseModel.setRes(0, true, "允许使用");
                }
                else
                {
                    res = ResponseModel.setRes(0, false, "不允许使用");
                }
            }
        }
        clsSharedHelper.WriteInfo(JsonConvert.SerializeObject(res));
    ...
    public void Policies()
    {
        ...
        try
        {
            ...
            rt = PostFunction("http://192.168.36.108:9301/discountForAll", jsondata);
    ...
    public void PushCloudOrd()//推送订单
    {
        string ID = Convert.ToString(HttpContext.Current.Request.Params["ID"]);
        string url = string.Format("{0}/PushCloudOrd?ID={1}", svrOrderUrl, ID);
        string rt = clsNetExecute.HttpRequest(url);
        clsSharedHelper.WriteInfo(rt);
    }
    //接受配送
    public void UpdOrderShop()
    {
        string ID = Convert.ToString(HttpContext.Current.Request.Params["ID"]);
        string StoreID = Convert.ToString(HttpContext.Current.Request.Params["StoreID"]);
        string CustomID = Convert.ToString(HttpContext.Current.Request.Params["CustomID"]);
        string url = string.Format("{0}/UpdOrderShop?ID={1}&StoreID={2}&CustomID={3}&", svrOrderUrl, ID, StoreID, CustomID);
        string rt = clsNetExecute.HttpRequest(url);
        clsSharedHelper.WriteInfo(rt);

    }
    //拒绝配送
    public void UpdNoOrderShop()
    {
        string ID = Convert.ToString(HttpContext.Current.Request.Params["ID"]);
        string StoreID = Convert.ToString(HttpContext.Current.Request.Params["StoreID"]);
        string CustomID = Convert.ToString(HttpContext.Current.Request.Params["CustomID"]);
        string url = string.Format("{0}/UpdNoOrderShop?ID={1}&StoreID={2}&CustomID={3}&", svrOrderUrl, ID, StoreID, CustomID);
        string rt = clsNetExecute.HttpRequest(url);
        clsSharedHelper.WriteInfo(rt);

    }
    //保存收货信息
    public void InsLogistics()
    {
        string data = Convert.ToString(HttpContext.Current.Request.Params["data"]);
        string url = string.Format("{0}/InsLogistics?data={1}", svrOrderUrl, data);
        string rt = clsNetExecute.HttpRequest(url);
        clsSharedHelper.WriteInfo(rt);
    ...
    /// 获取推送列表
    /// </summary>
    public void getSendCloudOrdList()
    {
        string data = Convert.ToString(HttpContext.Current.Request.Params["data"]);
        string url = string.Format("{0}/getSendCloudOrdList?data={1}", svrOrderUrl, data);
        string rt = clsNetExecute.HttpRequest(url);
        clsSharedHelper.WriteInfo(rt);

    }
    //获取配送列表
    public void getPushCloudOrdList()
    {
        string data = Convert.ToString(HttpContext.Current.Request.Params["data"]);
        string url = string.Format("{0}/getPushCloudOrdList?data={1}", svrOrderUrl, data);
        string rt = clsNetExecute.HttpRequest(url);
        clsSharedHelper.WriteInfo(rt);

    }
    //物流单号维护
    public void updOrderLogistics()
    {
        string data = Convert.ToString(HttpContext.Current.Request.Params["data"]);
        string url = string.Format("{0}/updOrderLogistics?data={1}", svrOrderUrl, data);
        string rt = clsNetExecute.HttpRequest(url);
        clsSharedHelper.WriteInfo(rt);
    }
    //获取待配送列表
    public void getNoSendOrdList()
    {
        string data = Convert.ToString(HttpContext.Current.Request.Params["data"]);
        string url = string.Format("{0}getNoSendOrdList?data={1}", svrOrderUrl, data);
        //   clsSharedHelper.WriteInfo(url);
        string rt = clsNetExecute.HttpRequest(url);
        clsSharedHelper.WriteInfo(rt);
    }
          //配送详情
    ...
    {
        string data = Convert.ToString(HttpContext.Current.Request.Params["billID"]);
        string url = string.Format("{0}GetOrderLogis?billID={1}", svrOrderUrl, data);
        //   clsSharedHelper.WriteInfo(url);
        string rt = clsNetExecute.HttpRequest(url);
        clsSharedHelper.WriteInfo(rt);
    }
