...
<script runat="server">
    ...
    {   
        ...
        if (Request.HttpMethod == "GET")
        {
            if (clsWXHelper.CheckUserAuth(ConfigKey, "openid"))
            {
                ...
                Response.Redirect("ShowInfoAll.aspx?mbopenid=" + Session["openid"].ToString());
    ...
    private void SetSelArea()
    {
        ...
        using (LiLanzDALForXLM dal = new LiLanzDALForXLM(conn))
        {
            ...
            string optionMb = "<option value='{0}'>{1}</option>";
            string strHtml = "<option value=''>请选择</option>";
            foreach (DataRow dr in dt.Rows)
            {
                strHtml += string.Format(optionMb, dr[0].ToString(), dr[1].ToString());
            }
            clsSharedHelper.DisponseDataTable(ref dt);
    ...
    private void SetSelMd()
    {
        string conn = clsWXHelper.GetWxConn();
        DataTable dt;
        string strInfo = "";

        string pValue = HttpUtility.UrlDecode(Request["selArea"].ToString());
        if (string.IsNullOrEmpty(pValue) || pValue == "") return;

        using (LiLanzDALForXLM dal = new LiLanzDALForXLM(conn))
        {
            strInfo = dal.ExecuteQuery(string.Format("SELECT DISTINCT mdid,StoreName FROM xn_t_BaseAreaStore WHERE tzid = 1 AND AreaID={0} ORDER BY StoreName", pValue), out dt);
            if (strInfo != "")
            {
                clsLocalLoger.WriteError(string.Concat("[我是型男]获取区域信息失败！错误：", strInfo));
                return;
...
<body>   
    ...
    <form name="form1" runat="server">
        ...
        <div class="item">
            ...
            <select id="selArea" onchange="SetSelStore()">
            </select> 
        </div> 
        <div class="item">
            <span>选&nbsp;择&nbsp;门&nbsp;店：</span>
    ...
    <script type="text/javascript">
        ...
        function SetSelArea() {
            ...
            $.ajax({
                type: "POST",
                url: "SigninAll.aspx",
                data: { action: "selArea", selProvinces: escape($("#selProvinces").val()) },
                timeout: 30000,
        ...
        //获取门店
        function SetSelStore() {
            if ($("#selArea").val() == "") return;
            $(".tip-txt").html("正在加载...");
            $(".loading-mask").show();
            $.ajax({
                type: "POST",
                url: "SigninAll.aspx",
                data: { action: "selStore", selArea: escape($("#selArea").val()) },
                timeout: 30000,
                error: function (XMLHttpRequest, textStatus, errorThrown) {           
            				$(".loading-mask").hide();
                    swal({ title: "-失败-", text: "获取门店出错!", type: "warning", html: true });
                },
                success: function (result) {
                    $("#selStore").html(result);           
            				$(".loading-mask").hide();
                }
            });
        }
        ...
        function SetShow() {
            ...
            if (mdid == "" || mdid == null) { swal({title:"-温馨提示-",text:"请选择门店！",type: "warning",html: true }); return; }
            else if (Cname == "") { swal({ title: "-温馨提示-", text: "请输入参赛者姓名！", type: "warning", html: true }); return; }
            else if (lenReg(Cname) > 50) { swal({ title: "-温馨提示-", text: "参赛者姓名不能超过50个字符，请重输！", type: "warning", html: true }); return; }
            else if (DPCname == "") { swal({ title: "-温馨提示-", text: "请输入搭配师姓名！", type: "warning", html: true }); return; }
            else if (lenReg(DPCname) > 50) { swal({ title: "-温馨提示-", text: "搭配师姓名不能超过50个字符，请重输！", type: "warning", html: true }); return; }
            else if (Phone == "") { swal({ title: "-温馨提示-", text: "请输入参赛者手机号码！", type: "warning", html: true }); return; }
            else if (lenReg(Phone) > 11) { swal({ title: "-温馨提示-", text: "参赛者手机号码不能超过11个字符，请重输！", type: "warning", html: true }); return; }
            else if (ISay == "") { swal({ title: "-温馨提示-", text: "请输入型男宣言！", type: "warning", html: true }); return; }
            else if (lenReg(ISay) > 72) { swal({ title: "-温馨提示-", text: "型男宣言不能超过72个字符，请重输！", type: "warning", html: true }); return; }
            else if (Idea == "") { swal({ title: "-温馨提示-", text: "请输入搭配理念！", type: "warning", html: true }); return; }
            else if (lenReg(Idea) > 72) { swal({ title: "-温馨提示-", text: "搭配理念不能超过72个字符，请重输！", type: "warning", html: true }); return; }
            else if (MyImgUrl0 == "") { swal({ title: "-温馨提示-", text: "请先上传参赛形象照！", type: "warning", html: true }); return; }
            else if (MyImgUrl1 == "") { swal({ title: "-温馨提示-", text: "请先上传搭配师工作照", type: "warning", html: true }); return; }
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    if (obj.errcode == "0") {
                        //swal(obj.errmsg);
                        window.location.href = "ShowInfoAll.aspx?bs=0&mbopenid=" + mbopenid;
            ...
            $("input[id=choosefile" + objIndex + "]").localResizeIMG({
                ...
                success: function (result) { 
                    $.ajax({
                        ...
                        error: function (XMLHttpRequest, textStatus, errorThrown) {
                            $(".loading-mask").hide();
                            swal({ title: "-失败-", text: "上传失败~" + errorThrown, type: "warning", html: true });
                        },
                        success: function (result) {
                            $(".loading-mask").hide();
                            var obj = eval("(" + result + ")");
        ...
        function choosefile(objIndex) {
            var mdid = $("#selStore").val(); 
            if (mdid == "" || mdid == null) { swal({ title: "-操作提示-", text: "请先选择赛区和门店！", type: "info", html: true }); return; }
            else $("#choosefile" + objIndex).click();
        } 
    </script>
</body>
</html>
