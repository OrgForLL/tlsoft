...
<script runat="server">
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        //if(bs=="0") tzFun = "window.location.href='List.aspx';";
        tzFun = "window.location.href='List.aspx';";    //无条件跳转回首页，不执行关闭.  By:xlm 20170615
        ...
        if (clsWXHelper.CheckUserAuth(ConfigKey, "openid"))
        {
            ...
            using (LiLanzDALForXLM sqlhelper = new LiLanzDALForXLM(conn))
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    {
                        Response.Write("<span style='font-size: 1em;'>该型男还未通过审核不能访问！！！</"+"span>");
                        Response.End();
                        return;
                    }
                    //参赛形象照
                ...
                {
                    clsSharedHelper.DisponseDataTable(ref dt);
                    clsLocalLoger.WriteError("[我是型男]该型男不存在！！！");
                    Response.End();
                }
                clsSharedHelper.DisponseDataTable(ref dt);
            }
