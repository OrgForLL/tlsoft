...
<body>
    <div class="container">
        ...
        <div class="store-detail">
            ...
            <div class="card">
                <span>线下卡</span>
                <p>
                    <img src="../../res/img/NewVip/tick.png" alt="">
                    已关联</p>
            </div>
        </div>
        <div class="line"></div>
        ...
        <div class="other" style="display: none;">
            <a href="./touchFollowV2.html?key=5" style="width: 100%; text-align: center;">关注微信公众号</a>
            <!-- <a id="vipinforeg" href="../vipweixin/vipInfoRegV1.aspx">注册新会员</a>
            <a id="vipbinding" href="../vipweixin/vipBinging_v3.aspx">关联线下卡</a> -->
        </div>
    </div>
    ...
    <div class="prompt" style="display: none;">
        <div class="alert" style="display: none;">
            ...
            <div class="content">
                <img class="pass" src="../../res/img/NewVip/pass.png" alt="">
                <img class="fail" src="../../res/img/NewVip/fail.png" alt="">
        ...
        (function (window, undefind) {
            ...
            function bindSelectorEvents() {
                var callhandle = function () {
                    $(".reason .onselected").removeClass("onselected");
                    $(this).addClass("onselected");
                    opinion = this.getAttribute('data-id');
                };

                $(".reason").on("click", 'li', callhandle);
            }
            ...
            function setQSWLogo() {
                if (configKey === '7') {
                    ...
                    document.querySelector('.other a').setAttribute('href', './touchFollowV2.html?key=7');        
                    /* document.querySelector('#vipinforeg').setAttribute('href', '../EasyBusiness/vipInfoReg.aspx');
                    document.querySelector('#vipbinding').setAttribute('href', '../EasyBusiness/vipBinging.aspx'); */
                }
            }
            ...
            function getGuideInfo() {
                ...
                if (OldSalerStr !== '') {
                    OldSaler = JSON.parse(OldSalerStr) || null;
                    if (NewSaler.id === OldSaler.id) {
                        setBindGuideInfo();
                        document.querySelector('.guide .title').innerText= '您已绑定该专属导购';
                        showOther();
                    } else {
                        setChangeGuideInfo();
                        getDisOpinions();
                        document.querySelector('.guide .title').innerText = '正在更换您的专属导购';
                        setSubmitText('立即更换');
                        bindChangeSubmit();
                    }
                } else {
                    setBindGuideInfo();
                    document.querySelector('.guide .title').innerText = '正在绑定您的专属导购';
                    setSubmitText('立即绑定');
                    getDisOpinions();
                    bindBindSubmit();
                }
            ...
            function setGuideInfo(dom, data) {
                var avatarUrl = '';
                if (data.avatar == '') {
                    avatarUrl = '../../res/img/NewVip/headImg.jpg';
                } else {
                    avatarUrl = headurl + data.avatar;
                }
                dom.querySelector('.headimg').style.backgroundImage = 'url(' + avatarUrl + ')';
                dom.querySelector('.headimg').style.backgroundSize = 'cover'; 
                dom.querySelector('.headimg').style.backgroundPosition = 'center';
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        ...
                        if (res.errcode === 0) {
                            ...
                            var text = res.errmsg;
                            showAlert({ text: text, err: true, title: '出错了', confirmCallBack: function() {
                                var err = !document.querySelector('.alert .err');
                                hidePrompt();
                                setBindGuideInfo(err);
                                showOther();
                                document.querySelector('.guide .title').innerText = '已绑定该专属导购';
                            }});
                        }
                    },
                    fail: function(err) {
            ...
            function showAlert(data) {
                ...
                if (callback) {
                    document.querySelector('.alert .btn a').removeEventListener('clicl', callback);
                }

                callback = data.confirmCallBack || function() {
                    window.location.href = './touchFollowV2.html?key=' + configKey;
                };

                document.querySelector('.alert .btn a').addEventListener('click', callback);
            }
            ...
            function showOther() {
                $('.container .submit').hide();
                $('.container .other').show();
            }
