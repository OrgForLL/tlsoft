    ...
    <script type="text/javascript">
        ...
        function loadList(){
            ...
            $.ajax({
                ...
                type: "POST",
                data: { orderInfo:myorderInfo, ksrq: $("#ksrq").val(), jsrq: $("#jsrq").val(), ccid: ccidval, approve: $("#approve").val(), splbid: splbid,khfl:$("#khfl").val(),kfbh:$("#kfbh").val() },
                dataType: "text",
        ...
        function drawTabChart(data, isdrawpie) {            
            ...
            for (var i = 0; i < data.rows.length; i++) {
                ...
                tableHtml += "<tr><td><a hre='#' onclick='splb(" + data.rows[i].khid + ")'>" + data.rows[i].khdm + "</a></td><td><a hre='#' onclick='khmx(" + data.rows[i].khid + ")'>" + data.rows[i].khmc + "</a></td><td>" + tSKU + "</td><td>" + tSl + "</td><td>" + fmoney(tJe, 2) + "</td><td>" + tJeScale + "%" + "</td></tr>";
                //构造图标参数
                if (tSKU > lSKU) {
        ...
        function research(isredrawpie) {        
            ...
            $.ajax({
                url: "KhReportData.aspx",
                type: "POST",
                data: {orderInfo: myorderInfo, ksrq: $("#ksrq").val(), jsrq: $("#jsrq").val(), ccid: ccidval, approve: $("#approve").val(), khfl: $("#khfl").val(), splbid: splbid, kfbh: $("#kfbh").val(), khdm: $("#khdm").val() , khmc: encodeURI($("#khmc").val())  },
                dataType: "text",
                contentType: "application/x-www-form-urlencoded; charset=UTF-8",
                timeout: 60000,
                error: function (XMLHttpRequest, textStatus, errorThrown) {
        ...
        function splb(xjkhid) {
        
            var ksrq = $("#ksrq").val();
            var jsrq = $("#jsrq").val();
            var splbmc=$("#splb").html();
            var paraccid=$("#clients").val();
            window.location.href = "SplbRep.aspx?FindKhid=" + xjkhid;
            // alert("正在开发中...");
        }
   ...
   <div id="load">
    ...
    <div class="t1">
        ...
        <div class="t1">
            ...
            <label>客户代码: </label><input type="text" id="khdm" />
            <label>客户名称: </label><input type="text" id="khmc" />  
