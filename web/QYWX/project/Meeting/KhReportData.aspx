...
<script runat="server">
    ...
    {
        String ksrq, jsrq, orderInfo, ccid, approve, khfl, splbid, kfbh, khdm, khmc;
        try
        {
            ...
            khdm = Convert.ToString(Request.Params["khdm"]);
            khmc = HttpUtility.UrlDecode(Convert.ToString(Request.Params["khmc"]));
        }
        catch
        {
        ...
        if (khfl != null && khfl != "")
        {
            if (khfl == "xk") filter = string.Concat(filter, " and a.khfl LIKE 'x[k,m,n]' ");
            else filter = " and a.khfl='" + khfl + "'";
        }
        ...
        {
            filter = string.Concat(filter, " and d.kfbh='", kfbh, "' ");
        }

        if (string.IsNullOrEmpty(khdm) == false) filter = string.Concat(filter, string.Format(" and a.khdm like '%{0}%'", khdm));

        if (string.IsNullOrEmpty(khmc) == false) filter = string.Concat(filter, string.Format(" and a.khmc like '%{0}%'", khmc));
        string mySql = string.Format(@"
                ...
                WHERE  a.khid={0} AND a.ty=0 {1} and b.rq>='{2}' and b.rq<dateadd(day,1,'{3}') {4}
                ...
                INNER JOIN YX_T_dddjb b ON a.khid=b.zmdid INNER JOIN dbo.YX_T_dddjmx c ON b.id=c.id
                INNER JOIN dbo.YX_T_Ypdmb d ON c.yphh=d.yphh
                WHERE  kh.ssid={0} AND kh.ty=0 AND kh.khid<>1 {1} and b.rq>='{2}' and b.rq<dateadd(day,1,'{3}') {4}
        ...
        if (khidArray[khidArray.Length - 1] == "1")
        {
            mySql = string.Format(@"SELECT a.khdm,a.khmc ,a.khid, SUM(c.je) je,SUM(c.sl) sl,COUNT(distinct d.SKU) sku
                        ...
                        INNER JOIN dbo.YX_T_Ypdmb d ON c.yphh=d.yphh
                        WHERE  a.ty=0 and b.rq>='{0}' and b.rq<dateadd(day,1,'{1}') {2} {3}
