...
<script runat="server">
    ...
    {
        string str_sql = "", json = "", lx = "", orderInfo = "", showmsg = "", ksrq = "", jsrq = "", approve = "", khfl = "", kfbh = "", filter = "", splbdm = "", splbmc = "";
        ...
        try
        {
            ...
            splbdm = Convert.ToString(Request.Params["splbdm"]);
            splbmc = HttpUtility.UrlDecode(Convert.ToString(Request.Params["splbmc"]));
        }
        catch
        {
            showmsg = @"{""err"":""【参数获取失败】""}";
        ...
        {
            if (khfl == "xk") filter = string.Concat(filter, " and kh.khfl LIKE 'x[k,m,n]' ");
            else filter = string.Concat(filter, " and kh.khfl='", khfl, "'");
        }

        if (string.IsNullOrEmpty(kfbh) == false)
        {
            filter = string.Concat(filter, " and c.kfbh='", kfbh, "'");
        }

        if (lx == "splb")
        {
         if (string.IsNullOrEmpty(splbdm) == false) filter = string.Concat(filter, string.Format(" and lb.dm like '%{0}%'", splbdm));
         if (string.IsNullOrEmpty(splbmc) == false) filter = string.Concat(filter, string.Format(" and lb.mc like '%{0}%'", splbmc));

            //  filter = khfl == "" ? "" : " and kh.khfl='" + khfl + "'";
            str_sql = @"select * from (select lb.dm,lb.mc,COUNT(distinct c.sku) SKU,SUM(isnull(b.sl,0)) sl,SUM(isnull(b.je,0)) je,lb.id from yx_t_dddjb a            
                        ...
                        where a.djlx=203 " + filter + " and a.tzid=1 and a.rq>='" + ksrq + @"' and a.rq<dateadd(day,1,'" + jsrq + @"') and kh.ccid + '-' like '" + ccid + @"'                    
    ...
    private void LoadKfbh()
    {
        using (LiLanzDALForXLM dal = new LiLanzDALForXLM(DHDBConstr))
        {
            string str_sql = @"select mc,dm from dbo.YF_T_Kfbh where tzid=1 order by dm desc";
            DataTable dt = null;
            string errinfo = dal.ExecuteQuery(str_sql, out dt);
            if (errinfo != "")
                clsSharedHelper.WriteErrorInfo(errinfo);
