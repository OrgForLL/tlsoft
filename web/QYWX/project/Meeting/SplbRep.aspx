...
<script runat="server">
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            FindKhid = Request.Params["FindKhid"];
    ...
    public void GetAuthMenu()
    {
                ...
                else
                {
                    ...
                    using (LiLanzDALForXLM dal = new LiLanzDALForXLM(DBConstr))
                    {
                        ...
                        if (errinfo == "")
                        {
                            ...
                            {
                                dr = dt.Rows[i];
                                sbCompany.AppendFormat(optionBase, dr["ccid"], dr["mdmc"], checkedAndSelected(Convert.ToString(dr["khid"])));
                            }//end for
                            ...
                            if (AppSystemKey != "" && AppSystemKey != "0")
                            {
                                ...
                                if (errinfo == "" && _dt.Rows.Count > 0)
                                {
                                    if (dt.Select("khid=" + Convert.ToString(_dt.Rows[0]["khid"])).Length == 0)
                                        sbCompany.AppendFormat(optionBase, _dt.Rows[0]["ccid"], _dt.Rows[0]["mdmc"], checkedAndSelected(Convert.ToString(_dt.Rows[0]["khid"])));
                                }//如果用户所属门店（贸易公司已被授权则不再添加否则会有两条记录）

                                clsSharedHelper.DisponseDataTable(ref _dt);  
                            }
        ...
        {             
            if (string.IsNullOrEmpty(FindKhid) == false)
            {
                sbCompany.AppendFormat(optionBase, "-1", "完整权限", "");
                AddsbCompany(ref sbCompany);
            }
            else
            {
    ...
    private string checkedAndSelected(string nowkhid){
        if (string.IsNullOrEmpty(FindKhid) == false && nowkhid == FindKhid)
        {
                return "selected";
        }else{
            return "";
    ...
    {
        if (string.IsNullOrEmpty(FindKhid))
        {
            return;
        }
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            if (ccid == "" || typeof (ccid) == "undefined") {
                //alert("对不起,您没有相关权限！");
                $("#load").hide();
                $("#mybody").removeClass("blur"); LoadKhflList();
                return;
            } else {
                ccid += "-%";
                $("#load").show();
                ...
                $.getJSON("SplbReportData.aspx", { ccid: ccid, lx: "splb", orderInfo: "je DESC", ksrq: $("#ksrq").val(), jsrq: $("#jsrq").val(), approve: $("#approve").val(), khfl: $("#kfbh").val() }, function (data) {
                    if (data.err == "") {
                        switchShow("show");
                        computeNums(data);
        ...
        function research(type) {
            ...
            setTimeout(function () {
                ...
                $.getJSON("SplbReportData.aspx", { ccid: ccid, lx: "splb", orderInfo: myorderInfo, ksrq: ksrq, jsrq: jsrq, approve: $("#approve").val(), khfl: $("#khfl").val(), kfbh: $("#kfbh").val(), splbdm: $("#splbdm").val(), splbmc: encodeURI($("#splbmc").val()) }, function (data) {
                    if (data.err == "") {
                        switchShow("show");
                        computeNums(data);
                        drawTabChart(data, "all");
        ...
        <div class="t1">
         ...
         <label>至：</label><input id="jsrq" type="date" value="<%=jsrq %>" />
          ...
          <div class="t1">
            ...
            <label>品类代码: </label><input type="text" id="splbdm" />
            <label>品类名称: </label><input type="text" id="splbmc" />        
