...
<script runat="server">
    ...
    public string mdid = "", mdmc = "", AppSystemKey = "", CustomerID = "", CustomerName = "";
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (clsWXHelper.CheckQYUserAuth(true))
        {
            ...
            else
            {
                ...
                {
                    DataTable dt;
                    string errinfo = dal.ExecuteQuery(string.Format(@"select dhbh from yx_t_dhryxx where id={0} and mdid>0", AppSystemKey), out dt);
                    if (errinfo != "")
                    {
                    ...
                    {
                        clsSharedHelper.WriteErrorInfo("未找到您的信息");
                        return;
                    }else if (Convert.ToString(dt.Rows[0]["dhbh"]) != dhbh)
                    {
                    ...
                    string str_sql = @"declare @ccid varchar(100);declare @curmdid int;
                                        select top 1 @ccid=ccid,@curmdid=a.mdid from yx_t_dhryxx a inner join yx_t_khb kh on a.khid=kh.khid where a.id='{0}';
