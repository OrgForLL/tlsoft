...
<script runat="server">
    ...
    private string DBConStr = "Data Source=192.168.35.10;Initial Catalog=tlsoft;User ID=ABEASD14AD;password=+AuDkDew";
    private string TestDBConStr = "Data Source=192.168.35.23;Initial Catalog=tlsoft;User ID=lllogin;password=rw1894tla";
    public string mdid = "", mdmc = "", AppSystemKey = "", CustomerID = "", CustomerName = "", dhbh = "201711", ParaMdid = "";
    
    protected void Page_Load(object sender, EventArgs e)
    {
        ParaMdid = Convert.ToString(Request.Params["mdid"]);
        //鉴权判断身份
        ...
        {            
            ...
            else
            {
                ...
                {
                    string sql = string.Format(@"select kh.ccid + '-'
                                                from yx_t_dhryxx a
                                                inner join yx_t_khb kh on a.khid=kh.khid and kh.ty=0
                                                inner join t_mdb md on md.khid=kh.khid and md.ty=0
                    ...
                    if (errinfo == "")
                    {
                        ...
                        {                             
                            if (ParaMdid != "" && ParaMdid != "0" && ParaMdid != null) {
                                sql = @"select TOP 1 a.mdid,a.mdmc,b.ccid
                                        from t_mdb a
                                ...
                                errinfo = dal.ExecuteQuerySecurity(sql, para, out dt2);
                                if (errinfo == "")
                                {
                                    if (dt2.Rows.Count == 0)
                                    {
                                        DisponseDT(ref dt);
                                        DisponseDT(ref dt2);
                                        clsWXHelper.ShowError("门店已停用！");
                                        return;
                                    }

                                    mdid = ParaMdid; // Convert.ToString(dt2.Rows[0]["mdid"]);
                                    mdmc = Convert.ToString(dt2.Rows[0]["mdmc"]);
                                    string _ccid = Convert.ToString(dt2.Rows[0]["ccid"]) + '-';
                                    bool IsOK = false;
                                    foreach (DataRow dr in dt.Rows)
                                    {
                                        if (_ccid.StartsWith(Convert.ToString(dr[0])))
                                        {
                                            IsOK = true;
                                            break;
                                        }
                                    ...
                                    if (!IsOK)
                                    {
                                        clsLocalLoger.WriteError(string.Format("越权访问！mdid:{0} _ccid:{1}", mdid, _ccid));
                                        clsWXHelper.ShowError("对不起，您已越权访问！");
                                    }
                                }
                                else
                                {
                                    clsLocalLoger.WriteError(string.Format("执行失败！错误:{0}", errinfo));                                    
                                    clsWXHelper.ShowError("执行错误！请稍后重试！");                                    
                                }
                            }//END有传入MDID参数
    ...
    private void DisponseDT(ref DataTable dt)
    {
        if (dt != null){
...
<body>
    ...
    <div class="header">
        <div class="center-translate">
            ...
            <div class="season">2017春夏季订货会</div>
        </div>
    </div>
    <div class="wrap-page">
        <!--主页-->
    ...
    <script type="text/javascript">
        var CurrentSite = "", mdid = "<%=mdid%>", CurrentID = "", LoadFlag = false, AppSystemKey = "<%=AppSystemKey%>", dhbh = "<%=dhbh%>", CustomersID = "<%=CustomerID%>";
        var mdmc = "<%=mdmc%>", formType = "", paraMdid = "<%=ParaMdid%>";        
    </script>
    <script type="text/javascript" src="../../res/js/meeting/JoinOrderJS.js?ver=1.1"></script>
</body>
</html>
