...
<img src="file:///C:\Users\curcry\AppData\Local\Temp\SGPicFaceTpBq\5228\1A6F11B6.png" />
    ...
    private const double eggWINPR = 0.2;//中奖因子
    ...
    private const Int32 MAXVISTPERSON=500;//最多访问人数
    private const Int32 TIMESLOT = 10;    //时长

    public static object vistLock = new object(); //访问锁
    ...
    public static DataTable roseUserDR()
    {
        ...
        if (_roseUserDT == null)
        {
            ...
            _roseUserDT.PrimaryKey = new DataColumn[] { _roseUserDT.Columns["wxid"] };
        }
        lock (vistLock)
        {
            DataRow[] drlist = _roseUserDT.Select("modifyTime <'" + DateTime.Now.AddSeconds(-TIMESLOT).ToString("yyyy-MM-dd HH:mm:ss") + "'");
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!canVisit(wxid))
        {
            clsSharedHelper.WriteInfo("网络拥堵,请稍后再访问...");
        }
        
     //   isTest = true;//测试模式...............................
        if (isTest && !isTestUser(wxid))
        {
            clsSharedHelper.WriteInfo(clsNetExecute.Error + "玫瑰心语正在测试中,请使用测试身份访问:" + wxid);
        }
    ...
    //限制访问人数 
    private Boolean canVisit(string wxid)
    {
        /*限制访问人数*/
        Boolean flag = false;
        DataTable activeDT = roseUserDR();
        DataRow dr_find = activeDT.Rows.Find(wxid);
        lock (vistLock)
        {
            if (dr_find == null && activeDT.Rows.Count < MAXVISTPERSON)
            {
                ...
                flag = true;
                clsLocalLoger.WriteInfo("允许新wxid:" + wxid);
            }
            else if (dr_find != null)
            {
                dr_find["modifyTime"] = DateTime.Now;
                flag = true;
                clsLocalLoger.WriteInfo("允许已存在wxid:" + wxid);
            }
            else
            {
                clsLocalLoger.WriteInfo("不允许新wxid:" + wxid);
            }
        }
        return flag;
    }

    private Boolean isTestUser(string wxid)
    {
        Boolean flag = false;
    ...
    private void ConsumeGameToken(string token, string wxid, string gameid)
    {
        using (LiLanzDALForXLM dal = new LiLanzDALForXLM(WXDBConstr))
        {
            ...
            {
                clsSharedHelper.WriteErrorInfo("亲,再接再厉吧~");
                return;
            }
