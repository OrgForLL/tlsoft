    ...
    public void getRank()
    {
        ...
        //{
            ...
            using (LiLanzDALForXLM zDal = new LiLanzDALForXLM(ZBConnStr))
            {
                ...
                {
                    DataTable dt = null;
                    string mySQLJe = "CASE WHEN ABS(djlb) = 1 THEN je ELSE 0-je END";     //根据类型取统计金额的判断。1　是零售单　2是退货单　-1是定制销售　　-2是定制退货　

                    string strSql = string.Concat(@"select top {0} ROW_NUMBER() OVER(ORDER BY sum(" , mySQLJe , ") DESC) as rowNum,ryid,yyy, CONVERT(INT,sum(" , mySQLJe , @")) zje,'' faceimg,'' myremark,'' LinkTitle,'' IconUrl,'' PageUrl
                                  from zmd_v_lsdjMX a WHERE djbs=1 and djlb=1 AND ryid > 0 ");
                    ...
                    if (errInfo == "")
                    {
                        if (dt.Rows.Count > 0)
                        {
                            using (LiLanzDALForXLM wxDal = new LiLanzDALForXLM(ConWX))
                            {
                                //查表 rs_t_yxmbje ，使用年月
                                strSql = @"
                           SELECT TOP 10 A.relateID AS ryid,C.avatar FROM wx_t_OmniChannelUser A 
                            ...
                            WHERE A.relateID IN ({0})";
                                ...
                                if (errInfo == "")
                                {
                                    ...
                                    DataRow dr1;
                                    DataRow[] dr1List;
                                    string defaultImage = "../../res/img/StoreSaler/defaulticon.jpg";
                                    ...
                                    //加载成功宣言
                                    strSql = @"
                               SELECT TOP 10 A.ryid,A.Remark,B.LinkTitle,B.IconUrl,B.PageUrl FROM wx_t_SaleRemark A 
                        ...
                        {
                            clsSharedHelper.WriteErrorInfo("找不到数据！");
                        }
                    }
                    else
                    {
