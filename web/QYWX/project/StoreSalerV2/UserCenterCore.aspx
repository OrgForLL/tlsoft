...
<script runat="server">
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        switch (ctrl)
        {
            case "saveFaceImage":
                ...
                clsSharedHelper.WriteInfo(SaveFaceImage(userID, formFile, rotate));
                break;
            case "saveNickname":
                string id = Convert.ToString(Request.Params["SystemKey"]);
                string nickname = System.Web.HttpUtility.UrlDecode(Request.Params["nickname"], System.Text.Encoding.Default);
                clsSharedHelper.WriteInfo(SaveNickname(id, nickname));
                break;
            default:
    ...
    public string SaveNickname(string id, string nickname)
    {
        using (LiLanzDALForXLM dal = new LiLanzDALForXLM(WXConnStr))
        {
            string errInfo = "";
            string strSQL = @"update wx_t_OmniChannelUser set Nickname=@Nickname where ID=@ID";
    ...
    public string SaveFaceImage(string userID, string formFile, string rotate)
    {
        //string strYM = DateTime.Now.ToString("yyyyMM");
        string path = HttpContext.Current.Server.MapPath("../../upload/StoreSaler/" + DateTime.Now.ToString("yyyyMM") + "/");
        string myPath = HttpContext.Current.Server.MapPath("../../upload/StoreSaler/" + DateTime.Now.ToString("yyyyMM") + "/my/");
        ...
        if (rt.Equals(""))
        {
            ...
            {
                string errInfo = "";
                string strSQL = @"update wx_t_customers set avatar=@FaceImg where ID=@userID";
                List<SqlParameter> paras = new List<SqlParameter>();
                paras.Add(new SqlParameter("@FaceImg", "upload/StoreSaler/" + DateTime.Now.ToString("yyyyMM") + "/my/" + filename));
                paras.Add(new SqlParameter("@userID", userID));
                errInfo = dal.ExecuteNonQuerySecurity(strSQL, paras);
                if (errInfo == "")
                {
                    return "upload/StoreSaler/" + DateTime.Now.ToString("yyyyMM") + "/my/" + filename;
                }
                else
                {
                    errInfo = "error:" + errInfo;
