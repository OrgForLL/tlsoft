<%@ Page Title="销售神器" Language="C#" MasterPageFile="../../WebBLL/frmQQDBaseV2.Master"
    ...
    { 
        if (!this.IsPostBack)
        {
            ...
            using (LiLanzDALForXLM dal = new LiLanzDALForXLM(connectstring))
            {
                ...
                string strSQL = string.Concat(@"SELECT TOP 1 C.NickName cname,A.avatar,A.mobile,A.email FROM wx_t_customers A
                                               INNER JOIN wx_t_AppAuthorized B ON A.ID = B.UserID
                                               INNER JOIN wx_t_OmniChannelUser C ON B.SystemID = 3 AND B.SystemKey = C.ID WHERE A.id=", customersid);
                ...
                { 
                    if (dt.Rows.Count > 0)
                    {
                        ...
                        if (relateID != null && relateID == "0")
                        {
                            ...
                            using (LiLanzDALForXLM zbdal = new LiLanzDALForXLM(zbConn))
                            {
                                //获取岗位，以便加载菜单
                                strSQL = string.Concat(@"SELECT TOP 1 B.mc,CAST(isnull(C.dm,0) as int)+1 as star 
                                    FROM rs_t_Rydwzl A 
                                    INNER JOIN rs_t_gwdmb B ON  A.gw = B.id LEFT OUTER JOIN dm_t_xzjbb C on A.zd = C.id 
                                    where A.id=", relateID);
                                strInfo = zbdal.ExecuteQuery(strSQL, out dt);
                                if (strInfo == "")
                                {
                                    if (dt.Rows.Count > 0)
                                    { 
                                        strGwmc = Convert.ToString(dt.Rows[0]["mc"]);
                                        strStar = Convert.ToString(dt.Rows[0]["star"]);

                                        if (strGwmc != "") strGwmc = string.Concat("(", strGwmc, ")");
                        ...
                        try
                        {
                            ...
                            Repeater1.DataSource = wx.MenuByRole(RoleID);
                            Repeater1.DataBind();
                        }
                        catch (Exception ex)
                        {
    ...
    <script type="text/javascript">
        ...
        $("input:file").localResizeIMG({
            ...
            success: function (result) {
                ...
                $.ajax({
                    url: "UserCenterCore.aspx?ctrl=saveFaceImage&rotate=" + oRotate,
                    type: "POST",
        ...
        function saveNickname() {
            ...
            $.ajax({
                url: "UserCenterCore.aspx?ctrl=saveNickname",
                type: "POST",
                data: { nickname: escape(nickname), SystemKey: SystemKey },
                dataType: "HTML",
                timeout: 30000,
                error: function (XMLHttpRequest, textStatus, errorThrown) {
                    showLoader("error", "昵称保存失败" + errorThrown);
                    setTimeout(function () {
                        $(".mask").hide();
                    }, 1000);
                },
