...
<script runat="server"> 
    ...
    private string CFSFConnStr =clsConfig.GetConfigValue("CFSF");
            ...
            else
            {
                using (LiLanzDALForXLM dal30 = new LiLanzDALForXLM(CFSFConnStr))
                {
                    string str_sql = @"select top 1 a.accountno,a.customername,isnull(b.deptname,'--') deptname,a.cardsnr 
                                        from tb_customer a 
                                        inner join tb_department b on a.deptno=b.deptno
...
<head runat="server">
    ...
    <style type="text/css">
        #main {
            ...
            background-image: url(../../res/img/storesaler/dinebg.png);
    ...
    <div class="wrap-page">
        <div class="page" id="main">
            <div class="mywrap center-translate">
                ...
                <p class="tips">请把二维码对准扫描仪</p>
                <p class="title"><span>注意事项</span></p>
                <p class="tip-item">利郎五里员工使用此功能，必须先开通企业号工卡系统</p>
    ...
    <script type="text/javascript">
        ...
        function getCode() {
            $.ajax({
                ...
                url: "fivelicanteencore.aspx?"+ Math.random(),
                data: {action: "QRcode"},
                success: function (msg) {
                  
                    var datajson = JSON.parse(msg);
                    if (datajson.errcode == 0) {
                         $("#qrcode").attr('src', "http://tm.lilanz.com/oa/project/StoreSaler/GetQrCode.aspx?code=" + datajson.data);
                    } else {
                        swal({
                            title: "",
                            text: datajson.errmsg,
                            type: "error",
                            confirmButtonColor: "#DD6B55",
                            confirmButtonText: "确定"
                        }, function (isConfirm) {
