...
<script runat="server">
    ...
    //string WXconnStr = "server='192.168.35.10';uid=ABEASD14AD;pwd=+AuDkDew;database=tlsoft";
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        switch (ctrl)
        {
            ...
            case "SaveNickname":
                ...
                break;
            case "savecname":
                string cname = System.Web.HttpUtility.UrlDecode(Request.Params["cname"], System.Text.Encoding.Default);
                clsSharedHelper.WriteInfo(saveCname(openid, cname));
                break;
            case "savebirthday":
                string birthday = Convert.ToString(Request.Params["birthday"]);
                clsSharedHelper.WriteInfo(saveBrithday(openid, birthday));
                break;
            default:
    ...
    public string saveBrithday(string openid, string birhtday)
    {

        string rt = "";
        string ID = getVipID(openid);
        if (ID.Contains(clsNetExecute.Error))
        {
            return ID;
        }
        ID = ID.Replace(clsNetExecute.Successed, "");

        using (LiLanzDALForXLM dal = new LiLanzDALForXLM(DBConnStr))
        {
            string errInfo = "";
            string strSQL = @"update YX_T_Vipkh set csrq=@birhtday where id=@id and (ISNULL(csrq,'1990-01-01')='1990-01-01' or khid<1 or csrq='1900-01-01')";
            List<SqlParameter> paras = new List<SqlParameter>();
            paras.Add(new SqlParameter("@birhtday", birhtday));
    ...
    public string saveCname(string openid, string cname)
    {
        string rt = "";
        string ID = getVipID(openid);
        if (ID.Contains(clsNetExecute.Error))
        ...
        {
            string errInfo = "";
            string strSQL = @"update YX_T_Vipkh set xm=@cname where id=@id";
            List<SqlParameter> paras = new List<SqlParameter>();
    ...
    private string getVipID(string wxopenid)
    {
        ...
        mysql = " SELECT TOP  10 isnull(vipID,0) vipID FROM wx_t_vipBinging WHERE wxOpenid=@wxopenid AND VipID>0 ";
        List<SqlParameter> paras=new List<SqlParameter>();
        ...
        using (LiLanzDALForXLM dal = new LiLanzDALForXLM(DBConnStr))
        {
            ...
            if (errInfo != "")
                ...
                rt = clsNetExecute.Error + "vipid不存在";
            else
                rt = clsNetExecute.Successed + dt.Rows[0]["vipID"];
    ...
    {
        string path = HttpContext.Current.Server.MapPath("../upload/VIPWEIXIN/" + DateTime.Now.ToString("yyyyMM") + "/");
        string myPath = HttpContext.Current.Server.MapPath("../upload/VIPWEIXIN/" + DateTime.Now.ToString("yyyyMM") + "/my/"); //压缩图路径
        ...
        if (rt.Equals(""))
        {
            using (LiLanzDALForXLM dal = new LiLanzDALForXLM(DBConnStr))
            {
                ...
                List<SqlParameter> paras = new List<SqlParameter>();
                paras.Add(new SqlParameter("@FaceImg", "upload/VIPWEIXIN/" + DateTime.Now.ToString("yyyyMM") + "/my/" + filename));
                paras.Add(new SqlParameter("@openid", openid));
                errInfo = dal.ExecuteNonQuerySecurity(strSQL, paras);
                if (errInfo == "")
                {
                    return "upload/VIPWEIXIN/" + DateTime.Now.ToString("yyyyMM") + "/my/" + filename;
                }
                else
                {
                    errInfo = "error:" + errInfo;
