            ...
            if (clsWXHelper.CheckUserAuth(ConfigKeyValue, "openid"))//20160330 薛灵敏改成使用 openid鉴权
            {
                ...
                else
                {
                    ...
                    using (LiLanzDALForXLM dalWX = new LiLanzDALForXLM(ConWX))
                    {
                        ...
                        //{
                            //                            string strSql = @" SELECT TOP 1 A.ID AS vsbID,A.SalerID,D.avatar,D.cname AS MyName FROM wx_t_VipSalerBind A 
                            //                                          INNER JOIN Rs_T_Ryjbzl B ON A.SalerID = B.ID 
                            //                                          INNER JOIN wx_t_AppAuthorized C ON C.SystemID = 3 AND C.SystemKey = CONVERT(VARCHAR(100),A.SalerID)
                            //                                          INNER JOIN wx_t_customers D ON D.ID = C.UserID
                            //                                           WHERE A.VipID=@vipid";
                            string strSql0 = @" SELECT TOP 1 A.ID AS vsbID,A.SalerID FROM wx_t_VipSalerBind A WHERE A.VipID=@vipid  ";
                            ...
                            lstParams.Add(new SqlParameter("@vipid", Convert.ToString(Session["vipid"])));
  ...
  <div class="container js_container">
    <div class="page">
        ...
        <div class="weui_btn_area">
            <a class="weui_btn weui_btn_primary" id="btnOK" href="javascript:SubmitOK();" id="showTooltips">确定</a>
        </div>    
    </div> 
  </div>
