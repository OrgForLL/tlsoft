...
<script runat="server">
    ...
    {
        if (configkey == null) configkey = clsConfig.GetConfigValue("CurrentConfigKey");
    ...
    public void bindVipkh(string phone,string wxopenid)
    {
        ...
        int recodetype = 0;
        DateTime createtime;
        mysql = string.Format(@"SELECT TOP 1 a.kh, a.id,a.yddh,ISNULL(a.khid,0) khid,ISNULL(a.mdid,0) mdid,a.jdrq FROM dbo.YX_T_Vipkh a  
                               LEFT JOIN yx_T_khb b ON a.khid=b.khid WHERE a.yddh=@phone and (khfl in({0}) OR a.khid=-1) order by a.jdrq desc",khfl);
        ...
        using (LiLanzDALForXLM dal = new LiLanzDALForXLM(DBConStr_tlsoft))
        {
            ...
            mysql = "SELECT top 1 * FROM dbo.wx_t_vipBinging WHERE vipid=@vipid and objectid=@objectid";
            paras.Clear();
            paras.Add(new SqlParameter("@vipid", vipid));
            ...
            {
                clsSharedHelper.WriteInfo(string.Format(rtmsg, "500", "", "绑定失败了,请核对信息后再绑定或填写技术人员。"));
                return;
            }
    ...
    public void sendVipSMS(string kh, string code)
    {
        ...
        //验证手机号码是否已注册
        mysql = string.Format("SELECT TOP 1 a.id,a.yddh,ISNULL(a.khid,0) khid,a.jdrq FROM dbo.YX_T_Vipkh a  LEFT JOIN yx_T_khb b ON a.khid=b.khid WHERE a.yddh=@phone and (khfl in({0}) OR a.khid=-1) order by a.jdrq desc",khfl);
        List<SqlParameter> paras = new List<SqlParameter>();
        ...
        using (LiLanzDALForXLM dal = new LiLanzDALForXLM(DBConStr_tlsoft))
        {
            ...
            {
                clsLocalLoger.Log("[vip绑定]短信发送：" + errInfo);
                clsSharedHelper.WriteInfo(string.Format(rtmsg, "500", "", "发送出错:网络出错了，稍等再试！"));
            }
            else
            {
    ...
    private string sendMeg(string phoneNumber,string code)
    {
        ...
        if (strInfo.Contains("成功") == false)
        {
            ...
            return string.Format(rtmsg, "500", "", "发送出错：网络超时，请稍等后再试");
        }
        else
        {
