    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (msgType == "event")
        {
            ...
            else if (xEvent == "subscribe" || xEvent == "SCAN")
            {
                ...
                //即日起至8月18日之前，推送活动提示
                if (DateTime.Now.Subtract(new DateTime(2018, 8, 18)).TotalSeconds > 0)
                {
                    clsSharedHelper.WriteInfo("success");
                }
                else
                {
                    backText(@"欢迎来到LILANZの爱情倾诉馆
 
你可以在这里讲出
“你在哪个瞬间认定他/她是对的人？”
你也可以在这里看到不同人身边的爱情故事
...
点击菜单栏——爱情倾诉馆——发帖留言即可");
            ...
            else if (xEvent == "create_map_poi_audit_info")//事件推送 --- 腾讯地图中创建门店的审核结果
            {
                create_map_poi_audit(px);
            }
            else if (xEvent == "add_store_audit_info")//事件推送 - 创建门店的审核结果
            {
                add_store_audit(px);
            }
            else if (xEvent == "modify_store_audit_info")//事件推送 --- 修改门店图片的审核结果
            {
                modify_store_audit(px);
            }else if (xEvent == "user_get_card")
            {
                clsLocalLoger.Log("[微信会员卡领卡回调]："+JsonConvert.SerializeObject(px));
            }
            else
            {   //尝试卡券的判断
                //微信推送过来的事件信息            
    ...
    private void CheckVIP()
    {
        using(LiLanzDALForXLM dal = new LiLanzDALForXLM(clsConfig.GetConfigValue("OAConnStr")))
        {
            ...
            {
                backPic(string.Concat("感谢您关注利郎男装"), "为了给您提供更加优质的利郎会员服务，系统需要先识别您的会员身份！点击 绑定（或注册）您的会员信息。"
    ...
    private void modify_store_audit(XmlDocument px)
    {
        string wxConn = clsWXHelper.GetWxConn();
        string status = px.GetElementsByTagName("status")[0].InnerText;
        string audit_id = px.GetElementsByTagName("audit_id")[0].InnerText;
        string reason= px.GetElementsByTagName("reason")[0].InnerText;

        using (LiLanzDALForXLM dal = new LiLanzDALForXLM(wxConn))
        {
            string mysql;
            List<SqlParameter> paras = new   List<SqlParameter> ();
            mysql =@"UPDATE wx_t_AppletStore SET modify_status=@modify_status,modify_reason=@modify_reason  
               WHERE base_id=@base_id";
            paras.Add(new SqlParameter("@modify_status",status));
            paras.Add(new SqlParameter("@modify_reason",reason));
            paras.Add(new SqlParameter("@base_id",audit_id));
            string errInfo = dal.ExecuteNonQuerySecurity(mysql, paras);
            if (errInfo != "") clsLocalLoger.Log("【修改门店的审核结果】："+errInfo);
        }
    }
    private void add_store_audit(XmlDocument px)
    {
        string wxConn = clsWXHelper.GetWxConn();
        string status = px.GetElementsByTagName("status")[0].InnerText;
        string audit_id = px.GetElementsByTagName("audit_id")[0].InnerText;
        string reason= px.GetElementsByTagName("reason")[0].InnerText;
        string is_upgrade= px.GetElementsByTagName("is_upgrade")[0].InnerText;
        string poiid= "0";
        if (status == "1") poiid = px.GetElementsByTagName("poiid")[0].InnerText;
        using (LiLanzDALForXLM dal = new LiLanzDALForXLM(wxConn))
        {
            string mysql;
            List<SqlParameter> paras = new   List<SqlParameter> ();
            mysql =@"UPDATE wx_t_AppletStore SET add_status=@add_status,add_reason=@add_reason,poiid=@poiid   
               WHERE base_id=@base_id";
            paras.Add(new SqlParameter("@add_status",status));
            paras.Add(new SqlParameter("@add_reason",reason));
            paras.Add(new SqlParameter("@poiid",poiid));
            paras.Add(new SqlParameter("@base_id",audit_id));
            string errInfo = dal.ExecuteNonQuerySecurity(mysql, paras);
            if (errInfo != "") clsLocalLoger.Log("【添加门店的审核结果】："+errInfo);
        }
    }

    private void create_map_poi_audit(XmlDocument px)//腾讯地图创建地图审核结果处理
    {
        clsLocalLoger.Log("【腾讯地图创建地图审核结果处理】");
        string wxConn = clsWXHelper.GetWxConn();
        string status = px.GetElementsByTagName("status")[0].InnerText;
        string audit_id = px.GetElementsByTagName("audit_id")[0].InnerText;
        string map_poi_id= px.GetElementsByTagName("map_poi_id")[0].InnerText;
        string name= px.GetElementsByTagName("name")[0].InnerText;
        string address= px.GetElementsByTagName("address")[0].InnerText;
        string latitude= px.GetElementsByTagName("latitude")[0].InnerText;
        string longitude = px.GetElementsByTagName("longitude")[0].InnerText;
        string remark = px.GetElementsByTagName("sh_remark")[0].InnerText;
        clsLocalLoger.Log("【腾讯地图创建地图审核结果处理】:map_poi_id="+map_poi_id);
        using (LiLanzDALForXLM dal = new LiLanzDALForXLM(wxConn))
        {
            string mysql;
            List<SqlParameter> paras = new   List<SqlParameter> ();
            mysql =@"UPDATE wx_t_AppletStore SET create_status=@create_status,sh_remark=@sh_remark,map_poi_id=@map_poi_id,name=@name,address=@address,latitude=@latitude,longitude=@longitude 
               WHERE base_id=@base_id";
            paras.Add(new SqlParameter("@create_status",status));
            paras.Add(new SqlParameter("@sh_remark",remark));
            paras.Add(new SqlParameter("@map_poi_id",map_poi_id));
            paras.Add(new SqlParameter("@name",name));
            paras.Add(new SqlParameter("@address",address));
            paras.Add(new SqlParameter("@latitude",latitude));
            paras.Add(new SqlParameter("@longitude",longitude));
            paras.Add(new SqlParameter("@base_id",audit_id));
            string errInfo = dal.ExecuteNonQuerySecurity(mysql, paras);
            if (errInfo != "") clsLocalLoger.Log("【创建门店的审核结果】："+errInfo);
        }
        ...
        using (LiLanzDALForXLM dal62 = new LiLanzDALForXLM(WXDBConstr()))
        {
            ...
            switch (eventType)
            {
                case "card_pass_check"://卡券审核通过事件
                    ...
                    str_sql += string.Format("update wx_T_vipCardInfo set card_status='CARD_STATUS_VERIFY_OK' where card_id='{0}';", cardid);
                    break;
                case "card_not_pass_check"://卡券审核未通过事件         
                    ...
                    str_sql += string.Format("update wx_T_vipCardInfo set card_status='CARD_STATUS_VERIFY_FAIL',refuseReason='{0}' where card_id='{1}';",px.GetElementsByTagName("RefuseReason")[0].InnerText, cardid);
                    break;
