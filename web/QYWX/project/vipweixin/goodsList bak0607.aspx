...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    ...
    <style type="text/css">
        ...
        .product-info, .product-stock, .product-detail1, .product-detail2 {
    ...
    <script type="text/javascript">
            ...
            wx.scanQRCode({
                ...
                success: function (res) {
                    ...
                    if(result.indexOf("http")>-1){ 
                        ...
                        if("<%=showType%>"=="2"){ 
                            goodsDetail(result.split(",")[0],"barCode");
                        }else{
                            goodsListSingle(result.split(",")[0],"barCode"); //待完善
                        }
        ...
        function goodsList(){ 
            ...
            $.ajax({
                ...
                type: "post",
                dataType: "text",
                data: { showType:<%=showType %>,mdid: <%=mdid%>,lastID:lastID },
				cache: false,
				timeout: 15000,
				error: function (e) {      
				    HideLoading();
				    ...
				    if (res.indexOf("Error：") > -1) {
				        ...
				        for (var i = 0; i < obj.rows.length; i++) {
				            if(obj.rows[i]["urlAddress"].toString()!=""){
				                ...
				                htmlStr+=temp.replace(/\$mid\$/g,obj.rows[i]["xh"].toString()).replace(/\$url\$/g,"css/loading_goods.png").replace("$productid$",obj.rows[i]["xh"].toString()).replace(/\$sphh\$/g,obj.rows[i]["sphh"].toString()).replace(/\$spmc\$/g,obj.rows[i]["spmc"].toString()).replace(/\$lsdj\$/g,obj.rows[i]["lsdj"].toString()).replace(/\$kc\$/g,obj.rows[i]["kc"].toString());
				            }
				        }  
				        if(lastID=="-1"){
				            $("#main").html(htmlStr);
        ...
        //加载详情页
        function goodsDetail(scanResult,scanType){            
            if("<%=showType%>"!="2"){		
		        $("#htmlList").hide();
            ...
            $.ajax({
				...
				success:function (res){
				    if(res.indexOf("Error")>-1){
				        ...
				        var htmlStr="<p class='title'>商品库存</p><ul>";
				        for(var i=0;i<obj.rows.length ;i++){
				            if(obj.rows[i]["sl"]>0){
            ...
            $.ajax({
				...
				success:function (res){
				    if(res.indexOf("Error")>-1){
				        ...
				        //商品卖点成份信息
				        var htmlStr="<p class='title'>商品参数</p><table>";
				        ...
				        htmlStr+=htmlTemp.replace(/\$name\$/g,"商品卖点").replace(/\$value\$/g,unescape(obj.rows[0]["cpmd"]));
				        htmlStr+=htmlTemp.replace(/\$name\$/g,"面料成份").replace(/\$value\$/g,obj.rows[0]["mlcf"].toString());
				        htmlStr+="</table>";
				        //商品图片
				        var PicStr="<p class='title'>商品图片</p>";
				        var PicTemp="<img src='$url$' onclick='javascript:previewImage(this);'/>";
				        for(var i=0;i<obj.rows.length;i++){
