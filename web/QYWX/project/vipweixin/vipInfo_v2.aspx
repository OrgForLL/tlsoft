...
<script runat="server">
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (vipid != 0)
        {
            ...
            string mysql = @"select a.kh,a.xm,a.yddh,b.wxHeadimgurl,b.wxOpenid,d.Nickname from YX_T_Vipkh a 
                             inner join wx_t_vipBinging b on a.id=b.vipid 
                             inner join wx_t_VipSalerBind c on a.id=c.VipID   
                             inner join wx_t_OmniChannelUser d on c.SalerID=d.id
                             where a.id = @vipid;";
            para.Add(new SqlParameter("@vipid", vipid));
            using (LiLanzDALForXLM dal = new LiLanzDALForXLM(DBConStr_tlsoft))
            {
                string eScal = dal.ExecuteQuerySecurity(mysql, para, out dt);
                if (eScal=="")
                {
...
<body>
       <form id="myInfo" runat="server">
        ...
        <script type="text/javascript" src="../../res/js/vipweixin/jquery-1.7.min.js"></script>
    <script type="text/javascript" src="../../res/js/vipweixin/LocalResizeIMG.js"></script>
    <script type="text/javascript" src="../../res/js/vipweixin/mobileBUGFix.mini.js"></script>
