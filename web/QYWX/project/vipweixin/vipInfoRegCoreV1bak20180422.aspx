...
<script runat="server">
   // private string weChatTest = "";
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
       ...
       // weChatTest = clsConfig.GetConfigValue("weChatTest");

        DBConStr_tlsoft = clsConfig.GetConfigValue("OAConnStr");
        rtmsg = @"{{""code"":""{0}"",""info"":""{1}"",""errmsg"":""{2}""}}";

        string ctrl=Convert.ToString(Request.Params["ctrl"]);
        //  string wxopenid = Convert.ToString(Request.Params["openid"]);
    ...
    /// 绑定已有vip卡号
    /// </summary>
    public void bindVipkh(string kh,string wxopenid)
    {
	 if (string.IsNullOrEmpty(kh))
        ...
        string errInfo, mysql, vipid = "0", khid = "0", mdid = "0",objectid="" ;
        int recodetype = 0;
        DateTime createtime;

        mysql = @"SELECT TOP 1 a.id,ISNULL(a.khid,0) khid,ISNULL(a.mdid,0) mdid
                    FROM dbo.YX_T_Vipkh a 
                    WHERE kh=@kh
                    ORDER BY a.id DESC ";
        ...
        using (LiLanzDALForXLM dal = new LiLanzDALForXLM(DBConStr_tlsoft))
        {
            ...
            mysql = "UPDATE wx_t_vipBinging SET vipid=@vipid,khid=@khid,mdid=@mdid WHERE wxopenid=@openid AND objectid IN(1,4)";
            paras.Clear();
            paras.Add(new SqlParameter("@vipid", vipid));
    ...
    {
        string errInfo, mysql, phoneNumber="",khid;
        mysql = "SELECT TOP 1 id,yddh,ISNULL(khid,0) khid FROM dbo.YX_T_Vipkh WHERE kh=@kh";//验证手机号码是否已注册
        List<SqlParameter> paras = new List<SqlParameter>();
        paras.Add(new SqlParameter("@kh", kh));
        DataTable dt;
        using (LiLanzDALForXLM dal = new LiLanzDALForXLM(DBConStr_tlsoft))
        {
            ...
            errInfo = sendSMS("您正在使用微信绑定利郎会员。验证码：" + code, phoneNumber,khid);
            if (errInfo.Contains("成功") == false)
            {
                clsLocalLoger.Log("[vip注册]短信发送：" + errInfo);
    ...
    private string registerVIP(string phoneNumber,string openid)
    {
        ...
        mysql = @"SELECT TOP 1 ID FROM wx_t_vipBinging WHERE wxOpenid=@openid AND VipID>0
                  UNION ALL
                  SELECT TOP 1 ID FROM dbo.YX_T_Vipkh WHERE kh=@phone";//验证微信、电话 是否已注册
        List<SqlParameter> paras = new List<SqlParameter>();
        ...
        using (LiLanzDALForXLM dal = new LiLanzDALForXLM(DBConStr_tlsoft))
        {
            ...
            mysql = @" DECLARE @VIPID INT, @khid INT, @mdid INT;
                       SELECT @khid = 0,@mdid = 0 ;
                       SELECT TOP 1 @khid=isnull(khid,0),@mdid=isnull(mdid,0) FROM wx_t_vipbinging WHERE wxopenid=@openid AND objectid in(1,4);
                       ...
                       SET @VIPID = @@identity;
                       UPDATE wx_t_vipBinging SET VIPID = @VIPID WHERE wxOpenid=@openid and objectid in(1,4) ;";
            paras.Clear();
            paras.Add(new SqlParameter("@phone", phoneNumber));
    ...
    private string sendMeg(string phoneNumber,string code)
    {
        ...
        string strInfo = sendSMS("您正在使用微信注册利郎会员。验证码：" + code, phoneNumber,khid);
        if (strInfo.Contains("成功") == false)
        {
            clsLocalLoger.Log("[vip注册]短信发送失败：wxopecid:" + Convert.ToString(Session["openid"]) + "|info:" + strInfo);
            return string.Format(rtmsg, "500", "", "发送出错了");
        }
        else
        {
            clsLocalLoger.Log("[vip注册]短信发送成功：wxopecid:" + Convert.ToString(Session["openid"]) + "|info:" + strInfo);
            mysql = "INSERT INTO wx_t_vipSMSCode(code,phoneNumber,wxopenid,sendTime,khid) VALUES(@code,@phoneNumber,@wxopenid,GETDATE(),@khid)";
            paras.Clear();
            paras.Add(new SqlParameter("@code", code));
            paras.Add(new SqlParameter("@phoneNumber", phoneNumber));
    ...
    public string sendSMS(string msg, string phoneNumber,string khid)
    {
        ...
        //string rt = clsNetExecute.HttpRequest(sendUrl, "", "get", "gb2312", 3000);
