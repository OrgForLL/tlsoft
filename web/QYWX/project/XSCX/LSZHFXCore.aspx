...
<script runat="server">
    ...
    public void GetCGData(string filterJSON) {
        ...
        switch (lx)
        {
            ...
            case "sphh":
                innerTables = string.Concat(innerTables.Replace(spdmb, ""), spdmb);
                str_sql = string.Format(str_sql, "sp.sphh", innerTables, "sp.sphh", filterStr);
                break;
            default:
        ...
        using (LiLanzDALForXLM dal_dd = new LiLanzDALForXLM(DBConn)) {
            ...
            if (errinfo == "")
            {
                ...
                if (dt_cg.Rows.Count > 0)
                {
                    ...
                    //获取品名
                    if (lx == "sphh") GetSpName(ref dt_cg);
    ...
    /// </summary>
    /// <param name="dt"></param>
    public void GetSpName(ref DataTable dt)
    { 
        StringBuilder sb = new StringBuilder();

        foreach (DataRow dr in dt.Rows)
        {
            sb.AppendFormat(",'{0}'", dr["sphh"]);
        }
        ...
        string strInfo = "";
        string strSQL = string.Format(@"SELECT sphh,spmc FROM yx_t_spdmb WHERE sphh IN ({0})",sb.ToString());
        using (LiLanzDALForXLM dal = new LiLanzDALForXLM(ZBDBConstr))
        {
            DataTable dt2;
            strInfo = dal.ExecuteQuery(strSQL, out dt2);
            ...
            {
                clsLocalLoger.WriteError(string.Concat("获取商品名称资料失败！错误：" , strInfo));
            }
    ...
    public DataTable GetLSSQL(string filterJSON) {
        ...
        string khb = " inner join yx_t_khb md on a.khid=md.khid and md.ty=0 ";
        string khb2 = " inner join yx_t_khb kh on md.ssid=kh.khid and kh.ty=0 ";
        ...
        switch (lx)
        {
            case "kh":
                ...
                _sql = @"select substring(a.ccid0,0,charindex('-',a.ccid0)) khid,upper(kh.khdm) khdm,kh.khmc,sum(a.sl) xssl,SUM(a.xsje) 'xsje'
                         from (
                           select a.khid,replace(md.ccid,'-1-','')+'-' ccid0,case when a.djlb in (-1,-2) then -1*a.sl else a.sl end sl,
                            (case when a.djlb in (-1,-2) then -1*a.je else a.je end) xsje          
                           from zmd_v_lsdjmx a                           
                           {0}
                           where a.djlb in (1,-1,2,-2) and a.djbs=1 {1}
                           ) a
                         inner join yx_t_khb kh on kh.khid=substring(a.ccid0,0,charindex('-',a.ccid0))
                         group by substring(a.ccid0,0,charindex('-',a.ccid0)),kh.khdm,kh.khmc";
                break;
            case "md":
                innerTables = string.Concat(innerTables.Replace(khb, ""), khb);
                _sql = @"select a.khid,md.khmc,sum(case when a.djlb in (-1,-2) then -1*a.sl else a.sl end) xssl,
                            sum(case when a.djlb in (-1,-2) then -1*a.je else a.je end) xsje          
                          from zmd_v_lsdjmx a                          
                          {0}
                          where a.djlb in (1,-1,2,-2) and a.djbs=1 {1}
                          group by a.khid,md.khmc";
                break;
            case "lb":
                innerTables = string.Concat(innerTables.Replace(spdmb, ""), spdmb);
                innerTables = string.Concat(innerTables.Replace(splbb, ""), splbb);
        ...
        if (khid != "")
        {
            khid = "-1-" + khid + "-%";
            innerTables = string.Concat(innerTables.Replace(khb, ""), khb);
            filterStr = string.Concat(filterStr, " and md.ccid+'-' like @khid");
            paras.Add(new SqlParameter("@khid", khid));
        }
        ...
        {
            innerTables = string.Concat(innerTables.Replace(khb, ""), khb);
            innerTables = string.Concat(innerTables.Replace(khb2, ""), khb2);
            filterStr = string.Concat(filterStr, " and kh.khfl=@khfl");
            paras.Add(new SqlParameter("@khfl", khfl));
        }
