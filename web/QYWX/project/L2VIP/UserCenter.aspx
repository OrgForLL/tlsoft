...
<script runat="server">
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        //Session["openid"] = "oU3wBj-CvOew3y4qxXPdSDvwIT78";
        //Session["vipid"] = "92997";
                
        //输出变量
        //string strTest = string.Format("A vipid:{0} openid:{1}", Session["vipid"], Session["openid"]);
        //clsSharedHelper.WriteErrorInfo(strTest);

        //CheckUserAuth 可能要重新重新开发  
        if (clsWXHelper.CheckUserAuth(ConfigKeyValue, "vipid"))
        {
            ...
            //string strTest = string.Format("BB  vipid:{0} openid:{1}", Session["vipid"], Session["openid"]);
            //clsSharedHelper.WriteErrorInfo(strTest); 
            ...
            using (LiLanzDALForXLM zdal = new LiLanzDALForXLM(DBConStr), wxdal = new LiLanzDALForXLM(ChatProConnStr))
            {
                ...
                paras.Add(new SqlParameter("@vipid", Convert.ToString(Session["vipid"])));
                ...
                errinfo = string.Concat(errinfo, wxdal.ExecuteQuerySecurity(str_sql2, paras2, out dt2));
                if (errinfo == "")
                {
                    if (dt.Rows.Count == 0)
                    {
                        clsWXHelper.ShowError("对不起，您还不是L2的会员。");
                    }
                    else if (dt2.Rows.Count == 0)
                    {
                        clsWXHelper.ShowError("对不起，您还未关注L2公众号。");
                    }
                    else
                    {
                        AddHT(VI, "vid", Convert.ToString(Session["vipid"]));
                        AddHT(VI, "wxnick", dt2.Rows[0]["wxnick"].ToString());
                        AddHT(VI, "xb", dt.Rows[0]["xb"].ToString());
                        AddHT(VI, "wxcity", dt2.Rows[0]["wxcity"].ToString());
                        AddHT(VI, "wxpro", dt2.Rows[0]["wxprovince"].ToString());
                        string headimg = dt2.Rows[0]["wxheadimgurl"].ToString().Replace("\\", "");
        ...
        { 
            using (LiLanzDALForXLM dal10 = new LiLanzDALForXLM(DBConStr))
            {
                ...
                else
                {
                    ...
                    string DBName = DBConStr;// = Convert.ToString(dt.Rows[0]["DBName"]).ToUpper();        //L2的DBName 没有特殊意义。
...
<body>
    <div class="guide-layer">
        <img class="center-translate" id="guide-pic" src="<%= VIP_resPath %>/img/vipweixin/guidepic.jpg" alt="" />
    ...
    <div id="main" class="wrap-page">
            ...
            <div class="page-head">
                <div style="position: absolute; top: 0; left: 0; height: 100%; width: 100%; padding-top: 20px; z-index: 205;">
                    ...
                    <div class="top-navs">
                        <ul class="tnavul">
                            <li>                                
                                <p class="vipval num"><img src="<%= VIP_resPath %>/img/vipweixin/bean-icon.png" /><%=VI["charmvalue"].ToString() %></p>
             ...
             <%--       <div class="iconcircle left" onclick="signToday()">
                        <img class="edit" src="<%= VIP_resPath %>/img/vipweixin/signicon.png" alt="" />
                    </div>--%>
                    <div class="iconcircle" onclick="javascript:window.location.href='MyInfos.aspx'">
                        <img class="sign" src="<%= VIP_resPath %>/img/vipweixin/editicon.png" alt="" />
                    </div>
                </div>
                ...
                <ul class="navul">
                    <li onclick="Redirect('myvip')">
                        <div class="item floatfix">
                            <img class="icon" src="<%= VIP_resPath %>/img/vipweixin/f-myvipcard.png" alt="" />
                            <span class="icontext">电子名片</span>
                        </div>
                    </li>
                    <li onclick="Redirect('wdzs')">
                        <div class="item floatfix subwaiting">
                            <img class="icon" src="<%= VIP_resPath %>/img/vipweixin/f-my.png" alt="" />
                            <span class="icontext">我的专属</span>
                        </div>
                    </li>
                    <li onclick="Redirect('tsjy')">
                        <div class="item floatfix subwaiting">
                            <img class="icon" src="<%= VIP_resPath %>/img/vipweixin/f-support.png" alt="" />
                            <span class="icontext">顾客心声</span>
                        </div>
                    </li>
                    <li onclick="Redirect('jfcx')">
                        <div class="item floatfix subwaiting">
                            <img class="icon" src="<%= VIP_resPath %>/img/vipweixin/f-points.png" alt="" />
                            <span class="icontext">利豆查询</span>
                        </div>
                    </li>
                    <li onclick="Redirect('xfcx')">
                        <div class="item floatfix">
                            <img class="icon" src="<%= VIP_resPath %>/img/vipweixin/f-xfcx.png" alt="" />
                            <span class="icontext">消费查询</span>
                        </div>
                    </li>
                    <li onclick="Redirect('njfcx')">
                        <div class="item floatfix">
                            <img class="icon" src="<%= VIP_resPath %>/img/vipweixin/f-newpoints.png" alt="" />
                            <span class="icontext">积分记录</span>
                        </div>
                    </li>
                    <li onclick="Redirect('scan')">
                        <div class="item floatfix subwaiting">
                            <img class="icon" src="<%= VIP_resPath %>/img/vipweixin/f-scan.png" alt="" />
                            <span class="icontext">商品扫描</span>
                        </div>
                    </li>
                    <li onclick="showGuide()">
                        <div class="item floatfix">
                            <img class="icon" src="<%= VIP_resPath %>/img/vipweixin/f-help.png" alt="" />
        ...
        <section class="page" id="points-explain">            
            <div class="content">
                ...
                <p class="answer">A：L2微信会员中心所显示的积分是新积分体系的积分，如果您会员卡所属门店并未加入“新积分体系”，则您的积分是旧积分。请以当地门店的积分查询结果为准。</p>
                ...
                <p class="answer">A：可以在全国任意一家加入“新积分体系”的利郎专卖店进行积分兑换。L2后续推出的积分商城使用新积分进行在线礼品兑换。</p>                
    ...
    <script type="text/javascript" src="<%= VIP_resPath %>/js/vipweixin/background-blur.min.js"></script>
...
//                    break;
    ...
    <script type="text/javascript" src="<%= VIP_resPath %>/js/vipweixin/usercenter-main.min.js"></script>
</body>
</html>
