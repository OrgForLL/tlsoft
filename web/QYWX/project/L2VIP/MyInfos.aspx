...
<script runat="server">
    ...
    private string VIP_WebPath = clsConfig.GetConfigValue("VIP_WebPath"); //资源目录
    protected void Page_Load(object sender, EventArgs e)
    {
        if (clsWXHelper.CheckUserAuth(ConfigKeyValue, "openid"))
        {
            ...
            using (LiLanzDALForXLM zdal = new LiLanzDALForXLM(DBConStr), wxdal = new LiLanzDALForXLM(ChatProConnStr))
            {
                ...
                string str_sql = @"SELECT TOP 1 khid,ISNULL(mdid,0) 'mdid',CONVERT(varchar(10),vip.tbrq,120) jointime,
                                            isnull(vip.kh,'--') vipkh,isnull(vip.yddh,'--') phone,
                ...
                {
                    if (dt.Rows.Count == 0)
                        clsWXHelper.ShowError(string.Concat("对不起，您不是", BrandName, "的会员。"));
                    else if (dt2.Rows.Count == 0)
                        clsWXHelper.ShowError(string.Concat("对不起，您还未关注", BrandName, "公众号。"));
                    else
                    {
    ...
    public string GetKHMC(string khid)
    {
        string rt = "--";
        using (LiLanzDALForXLM dal = new LiLanzDALForXLM(DBConStr))
        {
            string str_sql = "select top 1 khmc from yx_t_khb where khid=@khid;";
            List<SqlParameter> paras = new List<SqlParameter>();
            paras.Add(new SqlParameter("@khid", khid));
            DataTable dt = null;
            string errinfo = dal.ExecuteQuerySecurity(str_sql, paras, out dt);
            if (errinfo == "" && dt.Rows.Count > 0)
                rt = dt.Rows[0]["khmc"].ToString();
...
<body style="overflow: hidden;">
    ...
    <script type="text/javascript" src="<%= VIP_resPath %>/js/StoreSaler/LocalResizeIMG.js"></script>
    <script type="text/javascript" src="<%= VIP_resPath %>/js/StoreSaler/mobileBUGFix.mini.js"></script>
    <script type="text/javascript" src="<%= VIP_resPath %>/js/StoreSaler/binaryajax.min.js"></script>
    <script type="text/javascript" src="<%= VIP_resPath %>/js/StoreSaler/exif.min.js"></script>
    ...
    <script type="text/javascript">
        ...
        $("input:file").localResizeIMG({
            ...
            success: function (result) {
                ...
                $(".mask").hide();
                $.ajax({
                    url: "MyInfosCore.aspx?ctrl=SaveFaceImage&rotate=" + oRotate,
                    type: "POST",
                    ...
                    success: function (result) {
                        if (result.indexOf("error:") > -1) {
                            ...
                            var image = "url(<%= VIP_WebPath %>" + result + ")";
        ...
        function saveNickname() {
            ...
            $.ajax({
                url: "MyInfosCore.aspx?ctrl=SaveNickname",
                type: "POST",
