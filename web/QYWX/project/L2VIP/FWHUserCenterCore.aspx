...
<script runat="server">
    ...
    private string FXDB11 = clsConfig.GetConfigValue("OAConnStr");
        ...
        using (LiLanzDALForXLM dal10 = new LiLanzDALForXLM(DBConStr))
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    else if (DBName == "ERPDB") DBName = clsConfig.GetConfigValue("ERPConStr");
                    else DBName = DBConStr;
                    ...
                    str_sql = @"
                                ...
                                SELECT  YEAR(rq) nf ,
                                        MONTH(rq) yf ,
                                        CONVERT(INT,ISNULL(jfs,0)) changeval,
        ...
        using (LiLanzDALForXLM dal10 = new LiLanzDALForXLM(DBConStr))
        {
            string str_sql = @"
                                ...
                                from (
                                    ...
                                    where a.xtlx=1 and a.djbs=1 and a.kh=@vipkh
                                ) a

                                select a.*,isnull(b.ip,0) ip,isnull(b.op,0) op 
                                from #tmp a
                                left join (select yf,sum(inpoints) ip,sum(outpoints) op from #tmp group by yf) b on a.yf=b.yf AND a.nf=b.nf
    ...
    public void LoadConsumeRecords(string vipkh)
    {
        using (LiLanzDALForXLM dal11 = new LiLanzDALForXLM(FXDB11))
        {
            string str_sql = @"select a.id,convert(varchar(10),a.rq,120) rq,(CASE WHEN a.djlb > 0 THEN CONVERT(VARCHAR(10),sskje) ELSE CONVERT(VARCHAR(10),0-sskje) + '(退货）' END) 'sskje'
                                ,md.mdmc,convert(varchar(8),a.rq,112)+a.djh ordernum
                                from zmd_t_lsdjb a
