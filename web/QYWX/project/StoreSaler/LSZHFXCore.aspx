...
<script runat="server">
    ...
    public DataTable GetLSSQL(string filterJSON) {
        ...
        if (khid != "")
        {
            string ccid = getCcid(Convert.ToInt32(khid));
            //clsLocalLoger.WriteInfo("ccid=" + ccid);
            if (ccid.Replace("-","").Length + 2 == ccid.Length)
            {
                //clsLocalLoger.WriteInfo("1"); 
                innerTables = string.Concat(innerTables.Replace(mdb, ""), mdb);
                innerTables = string.Concat(innerTables.Replace(khgxb, ""), khgxb);
                filterStr = string.Concat(filterStr, " and gx.myid = @khid ");
                paras.Add(new SqlParameter("@khid", khid));
            }else
            {
                //clsLocalLoger.WriteInfo("2");
        ...
        if (ksrq != "" && jsrq != "")
        {
            ...
            filterStr = string.Concat(filterStr, string.Format(" AND DATEDIFF(day,'{0}',a.rq) >= 0 and DATEDIFF(day,a.rq,'{1}') > 0 "
                , Convert.ToDateTime(ksrq).ToString("yyyy-MM-dd"), Convert.ToDateTime(jsrq).AddDays(1).ToString("yyyy-MM-dd")));             
        }
        ...
        //clsLocalLoger.WriteInfo("SQL2:" + _sql);
    ...
    public void GetMDLSSQL(string filterJSON)
    {
        ...
        //innerTables = string.Concat(innerTables.Replace(mdb, ""), mdb);
        //innerTables = string.Concat(innerTables.Replace(khb, ""), khb);
        _sql = @"select a.mdid khid,md.mdmc khmc,m.myid,COUNT(DISTINCT a.id) djs,sum(case when a.djlb in (-1,-2) then -1*a.sl else a.sl end) xssl,
        ...
        */
        ...
        if (khid != "")
        {
            ...
            {
                //clsLocalLoger.WriteInfo("1");
                filterStr = string.Concat(filterStr, " and m.myid = @khid ");
                paras.Add(new SqlParameter("@khid", khid));
            }else
            {
                //clsLocalLoger.WriteInfo("2");
        ...
        //WriteLog(_sql + "\r\n" + filterJSON); 
        //clsLocalLoger.WriteInfo("_sql1=" + _sql);
