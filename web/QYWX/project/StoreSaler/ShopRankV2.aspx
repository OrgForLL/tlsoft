<%@ Page Title="琅琊榜" Language="C#" MasterPageFile="../../WebBLL/frmQQDBase.Master" AutoEventWireup="true" %>
...
<script runat="server">
    ...
    {
        clsWXHelper.CheckQQDMenuAuth(3);    //检查菜单权限
        ...
        {
            string strSQL = string.Format(@"SELECT TOP 1 RelateID FROM wx_t_OmniChannelUser A
        ...
        if (mdid == "") mdid = "0"; //clsWXHelper.ShowError("找不到门店信息！");
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <div class="header">
        <a href="javascript:void(0)" class="nav-item current-nav" onclick="changeArea('shop')" id="shop">本店</a>
        <a href="javascript:void(0)" class="nav-item" onclick="changeArea('area')" id="area">区域</a>
        <a href="javascript:void(0)" class="nav-item" onclick="changeArea('province')" id="province">全省</a>
    ...
    <script type="text/javascript">
        ...
        function getRank(divname, rankname, datamonth) { 
            ...
            if (ShowMyInfo(divname + datamonth) == true) { //如果能成功显示排名，则不继续查询数据
                $("#" + divname + rankname).css("opacity", 1);

                LoadAllSale(mdid, datamonth);    //加载总业绩
                ...
                success: function (data) {
                    ...
                    if (len > 0) {
                        ...
                        for (var i = 0; i < len; i++) {
                            ...
                            if (i < 3) {
                                rankhtml += "<div class=\"top3-item Winner" + (i + 1).toString() + " animated bounceIn\">\n" +
                        ...
                        //调整位置并放大
                        $(".list.Active .Winner1").insertAfter($(".list.Active .Winner2"));
                        $(".list.Active .Winner1").css("zoom", "1.2");
        ...
        function LoadAllSale(mdid, datamonth) {
            if (NowRankID != "shop" || "<%= RoleID %>" == "1") {
                $(".AllSale").fadeOut();
                return;
            } else {
                $(".AllSale").fadeIn();
            ...
            $.ajax({
                ...
                datatype: "html",
                url: "ShopRankCore.aspx",
                data: { "ctrl": "LoadAllSale", "mdid": mdid, "datamonth": datamonth, "ref": timestamp },
                cache: false,
                contentType: "application/x-www-form-urlencoded; charset=utf-8",
                success: function (data) {
                    //showLoader("successed", "加载完毕！");
                    if (data.indexOf("Error:") == 0) {
                        ...
                        var myAllSale = GetWan(data) + "元";
                        $("#MyAllSale").html(myAllSale);
                        $(".AllSale").fadeIn();
                    } 
                },
                error: function (XMLHttpRequest, textStatus, errorThrown) {
        ...
        $(".list").on("click", ".top3-item,.ranklist>li", function () {
            ...
            //在店长身份下该行本店人员的业绩查询入口
            var mdid = "<%= mdid %>";
            if ("<%= RoleID %>" == "1" || $data.attr("mdid") == 0 || $data.attr("mdid") != mdid) {
