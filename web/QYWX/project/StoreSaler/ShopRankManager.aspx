...
<script runat="server">
    ...
    {         
        ...
        {
            string strSQL = string.Concat(@"SELECT A.khid,A.khmc mdmc,A.ssid,0 mdid FROM yx_t_khb A 
    ...
    private void CalKhlist(ref DataTable dt)
    {
        if (dt != null)
        {
            string optionBase = @"<option value=""{0}|{1}|{2}"">{3}</option>";
            StringBuilder sbOption = new StringBuilder();
            foreach (DataRow dr in dt.Rows)
            {
                sbOption.AppendFormat(optionBase, dr["khid"], dr["ssid"], dr["mdid"], dr["mdmc"]);
            }
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <div class="header">
        <span class="nav-item2 current-nav" id="province">
            <div class="storeicon">
    ...
    <script type="text/javascript">
        ...
        function getRank(divname, rankname, datamonth) {
            ...
            var interfaceDivName = "";
            if (divname == "province") {    //如果是按省份（贸易公司）
                var selectVal = $("#khsel").val();
                 
                if (typeof (selectVal) == undefined || selectVal == "") { showLoader("error", "必须选择一个贸易公司！"); return; }
                else {
                    var selval = selectVal.split("|");

                    khid = selval[0];
                    if (selval[1] == "1") interfaceDivName = "province";
                    else {
                        interfaceDivName = "shop";
                        mdid = selval[2];
                    }
                }
            } else {
                interfaceDivName = divname;
            }
            ...
            if (ShowMyInfo(divname + datamonth) == true) { //如果能成功显示我的排名，则不继续查询数据
                ...
                LoadAllSale(khid, datamonth);    //加载总业绩
            ...
            $.ajax({
                ...
                data: { "ctrl": "getRank", "ryid": ryid, "mdid": mdid, "khid": khid, "datatype": interfaceDivName, "datamonth": datamonth, "ref": timestamp },
                cache: false,
        ...
        function LoadAllSale(khid, datamonth) {
            ...
            $.ajax({
                ...
                datatype: "html",
                url: "ShopRankCore.aspx",
                data: { "ctrl": "LoadAllSale", "khid": khid, "datamonth": datamonth, "ref": timestamp },
                cache: false,
                contentType: "application/x-www-form-urlencoded; charset=utf-8",
                success: function (data) {
                    //showLoader("successed", "加载完毕！");
