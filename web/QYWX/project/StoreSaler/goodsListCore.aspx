...
<script runat="server">  
            ...
            case "1":
                switch (ctrl)
                {
                    case "goodsList":
                        ...
                        if (scanResult == "")
                            rt = goodsList(tzid,lastId);
                        else
                            rt = goodsListSingle(tzid, scanResult, "", lastId);
                        break;
                    case "goodsfilter":
                        scanResult = Convert.ToString(Request.Params["scanResult"]);
                    ...
                    case "goodsListSingle":
                        ...
                        scanType = Convert.ToString(Request.Params["scanType"]);
                        lastId = Convert.ToString(Request.Params["lastID"]);                        
                        rt = goodsListSingle(tzid, scanResult, scanType, lastId);
                        break;
                    case "goodsDetail":
                        scanResult = Convert.ToString(Request.Params["scanResult"]);
                        scanType = Convert.ToString(Request.Params["scanType"]);
                    ...
                    case "goodsStock":
                        scanResult = Convert.ToString(Request.Params["scanResult"]);
                        scanType = Convert.ToString(Request.Params["scanType"]);
                        rt = goodsStock(tzid, scanResult, scanType);
                        break;
    ...
    //主商品信息
    public string goodsList(string tzid, string lastId)
    {
        ...
        if (lastId == "-1")
        {
            strsql = @" select top 12 c.kfbh,b.sphh,max(c.ypmc) as spmc,max(c.yphh) as yphh,max(c.lsdj) as lsdj,sum(b.sl0)  as kc,'' as urlAddress
                           ...
                           from yx_t_spkccmmx b 
                           inner join YX_T_ypdmb c on b.sphh=c.sphh and c.tzid=1 
                           where b.tzid=@tzid and c.ztbs=1 group by b.sphh,c.kfbh order by c.kfbh desc,b.sphh desc;  
                        ";
            param.Add(new SqlParameter("@tzid", tzid));
        }
        else
        {
        ...
        {
            string dbConn = MDal.GetDBName(Convert.ToInt32(tzid));

            if (dbConn == "FXDB") MDal.ConnectionString = FXDBConStr;
            else if (dbConn == "ERPDB") MDal.ConnectionString = ERPConStr; 
    ...
    //扫描及查找商品信息列表	
    public string goodsListSingle(string tzid, string scanResult, string scanType, string lastid)
    {
        string errinfo, strsql, rt; 
        string sphh = GetSPHH(scanResult, scanType);
        
        strsql = @" 
				select c.kfbh,c.sphh,max(c.spmc) as spmc,max(c.yphh) as yphh,max(c.lsdj) as lsdj,sum(b.sl0)  as kc,'' as urlAddress ,row_number()over(order by c.sphh desc) as xh into #temp
         	    from  YX_T_spdmb c 
         	    left join yx_t_spkccmmx b on c.sphh=b.sphh AND b.tzid = @tzid 
				where c.tzid=1 {0} group by c.sphh,c.kfbh order by c.kfbh desc,c.sphh desc;
                select top 12 kfbh,sphh,spmc,yphh,lsdj,kc,urlAddress,xh from #temp where xh>@lastId;
                drop table #temp;";
//        if (sphh.Length > 3)
//        {
...
//        }
        ...
        if (sphh != "") strsql = string.Format(strsql, " AND c.sphh=@sphh ");
        else
        {
            sphh = scanResult;
            strsql = string.Format(strsql, " AND c.sphh like @sphh+'%' ");
        }

        //clsLocalLoger.WriteInfo("scanResult=" + scanResult);
        //clsLocalLoger.WriteInfo("scanType=" + scanType);
        //clsLocalLoger.WriteInfo("tzid=" + tzid);
        //clsLocalLoger.WriteInfo("sphh=" + sphh);
        //clsLocalLoger.WriteInfo("lastid=" + lastid);
    ...
    /// <summary>
    /// 返回商品货号。薛灵敏 于 20160820 临时新增
    /// </summary>
    /// <param name="code">加密码</param>
    /// <param name="scanType">扫描方式。这个参数在正常设计下是不应被传入到商品详情页的</param>
    /// <returns></returns>
    public string GetSPHH(string code, string scanType)
    {
        string errinfo = "";
        string strsql = "";
        using (LiLanzDALForXLM MDal = new LiLanzDALForXLM(OAConnStr))
        {
            if (scanType == "qrCode")
                strsql = @"declare @strGood varchar(30) select @strGood=dbo.f_DBPwd(@sphh) 
                            select @strGood = (CASE WHEN (LEN(@strGood) > 13) THEN SUBSTRING(@strGood, 1, LEN(@strGood) - 6) ELSE @strGood END) 
                            select top 1 a.sphh from yx_t_tmb a where tm=@strGood ";
            else if (scanType == "barCode")
                strsql = @"declare @strGood varchar(30) select @strGood = SUBSTRING(@sphh, 1, LEN(@sphh) - 6)
												select top 1 a.sphh from yx_t_tmb a where tm=@strGood ";
            else
                return "";
            ...
            object objsphh = "";
            List<SqlParameter> param = new List<SqlParameter>();
            param.Add(new SqlParameter("@sphh", code));
            errinfo = MDal.ExecuteQueryFastSecurity(strsql, param, out objsphh);
            if (errinfo != "")
            {
                clsLocalLoger.WriteError(string.Concat("用码", scanType, " 转换商品货号失败！错误：", errinfo));
                return "";
            }
            else
            {
                return Convert.ToString(objsphh);
            }
        }
    }
    ...
    public string goodsDetail(string scanResult, string scanType)
    {
		...
		string strsql = @" 
						   ...
						   where sp.tzid=1 AND sp.sphh = @sphh
		                ";
...
//        }
        ...
        //clsLocalLoger.WriteInfo("goodsDetail .scanResult=" + scanResult);
        //clsLocalLoger.WriteInfo("scanType=" + scanType); 
        //clsLocalLoger.WriteInfo("sphh=" + sphh); 
        //clsLocalLoger.WriteInfo("strsql=" + strsql);
    ...
    //商品库存
    public string goodsStock(string tzid, string scanResult, string scanType)
    {
        string errorinfo, rt;
        ...
        string strsql = @"  
                        ...
                        left join(
                            ...
                            where a.tzid=@tzid AND a.sphh = @sphh
