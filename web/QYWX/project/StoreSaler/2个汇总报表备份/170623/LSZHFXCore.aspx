...
<script runat="server">
    ...
    public void GetMDLSSQL(string filterJSON)
    {
        ...
        List<SqlParameter> paras = new List<SqlParameter>(); 
                innerTables = string.Concat(innerTables.Replace(mdb, ""), mdb);
                innerTables = string.Concat(innerTables.Replace(khb, ""), khb);
                _sql = @"select a.khid,m.khmc,COUNT(DISTINCT a.id) djs,sum(case when a.djlb in (-1,-2) then -1*a.sl else a.sl end) xssl,
                    CONVERT(INT,sum(case when a.djlb in (-1,-2) then -1*a.je else a.je end)) xsje,
                    CONVERT(INT,sum(case when a.djlb in (-1,-2) then -1*a.sl*a.bj else a.sl*a.bj end)) bjje          
                    from zmd_v_lsdjmx a    
                    inner join yx_t_khb m on a.khid=m.khid                      
                    {0}
                    where a.djlb in (1,-1,2,-2) and a.djbs=1 {1}
