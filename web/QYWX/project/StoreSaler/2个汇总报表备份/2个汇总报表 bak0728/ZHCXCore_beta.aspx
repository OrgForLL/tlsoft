...
<script runat="server">
    ...
    private string CXDBConstr = "server='192.168.35.32';database=tlsoft;uid=lllogin;pwd=rw1894tla";
    ...
    public void GetDHData(string filterJSON) {        
        ...
        string str_sql = @" select {0},sum(b.sl) ddsl
                            from yx_t_dddjb a
                            inner join yx_t_dddjmx b on a.id=b.id                                
                            {1}                                
                            where a.djlx=201 and a.djbs=1 {3} --and a.shbs=1 and a.qrbs=1
                            group by {2} order by sum(b.sl) desc";
        ...
        switch (lx)
        {
            case "kh":
                ...
                innerTables = string.Concat(innerTables.Replace(khb, ""), khb);
                str_sql = string.Format(str_sql, "convert(varchar(10),a.khid) khid,upper(kh.khdm) khdm,kh.khmc,kh.khfl", innerTables, "a.khid,kh.khdm,kh.khmc,kh.khfl", filterStr);
                break;
            case "md":
                innerTables = string.Concat(innerTables.Replace(khb, ""), khb);
                str_sql = string.Format(str_sql, "a.khid,kh.khmc", innerTables, "a.khid,kh.khmc", filterStr);
                break;
            case "lb":
                innerTables = string.Concat(innerTables.Replace(spdmb, ""), spdmb);
            ...
            case "sphh":
                ...
                if (mdid != "")
                {
                    ...
                    //查看货号时多了周转量与备货量
                    str_sql = @"select {0},sum(b.sl) ddsl into #zb
                            from yx_t_dddjb a
                            inner join yx_t_dddjmx b on a.id=b.id                                
                            {1}                                
                            ...
                            select a.sphh,isnull(a.tcl,0)-isnull(b.ddsl,0) zzl,c.bhl into #zzbh
                            from #sphh s
                            left join (
                              select a.sphh,sum(b.sl0) tcl 
                              from #sphh s inner join yx_t_tcjhb a on s.sphh=a.sphh inner join yx_t_tcjhcmmx b on a.id=b.id 
                              where a.tzid=1 and a.shbs=1 group by a.sphh
                            ) a on s.sphh=a.sphh
                            left join (
                              select a.sphh,sum(a.sl) ddsl  
                              from #sphh s
                              inner join yx_v_dddjmx a on a.sphh=s.sphh
                              where a.djlx=201 and a.djbs=1 group by a.sphh
                            ) b on a.sphh=b.sphh
                            left join (
                              select a.sphh,sum(a.sl0-a.dbdf0-a.qtdf0) bhl 
                              from #sphh s
                              inner join YX_T_Spkccmmx a on a.sphh=s.sphh
                              where a.tzid=1 group by a.sphh
                            ) c on a.sphh=c.sphh
        ...
        using (LiLanzDALForXLM dal_dd = new LiLanzDALForXLM(DBConn)) {
            ...
            if (errinfo == "")
            {
                ...
                clsSharedHelper.WriteErrorInfo("GetDHData" + errinfo + "|" + DBConn + "|" +filterJSON);
        }//end using
    }
    ...
    public void GetSphhCmmx(string type,string sphh) {
        ...
        if (type == "zzl")
        {
            _sql = @"select a.cm cmdm,SUM(ISNULL(tc.sl,0)-ISNULL(dd.sl,0)) cmsl
                    ...
                    FROM yx_t_tcjhb a INNER JOIN yx_t_tcjhcmmx b ON a.id=b.id WHERE a.sphh=@sphh and a.shbs=1
                    GROUP BY b.cmdm) tc ON a.cmdm=tc.cmdm
                    LEFT JOIN (
                    SELECT  cmdm,SUM(sl0) AS sl
                    FROM dbo.yx_v_dddjcmmx WHERE sphh=@sphh AND djlx=201 and djbs=1
        ...
        else if (type == "bhl")
        {
            _sql = @"select a.cm cmdm,SUM(ISNULL(b.sl0,0)) cmsl
                    from  yx_t_cmzh a
                    inner join YX_T_Spdmb c on a.tml=c.tml and c.sphh=@sphh and c.tzid=1
                    left join(SELECT sl0-dbdf0-qtdf0 AS sl0,cmdm FROM dbo.YX_T_Spkccmmx WHERE ckid not in (9325,6671,7013) and sphh=@sphh AND tzid=1) b on a.cmdm=b.cmdm
                    where a.tzid=1
    ...
    private string GetDBConstr(string khid,string mdid) {        
        using (LiLanzDALForXLM dal = new LiLanzDALForXLM(ZBDBConstr)) {
            string str_sql="";
            if (khid != "" && khid != "0") {
                str_sql = "select top 1 dbserver+'.'+dbname from yx_t_khb where khid='" + khid + "' and ssid=1";                
            }
            else if (mdid.Length != 0 && mdid != "0") {
                str_sql = @"select top 1 b.dbserver+'.'+b.dbname
                            from yx_t_khb a
                            inner join yx_t_khb b on a.ssid=b.khid and b.ssid=1
                            where a.khid='" + mdid + "'";                
            }
            else
                return "";
            ...
            if (errinfo == "")
            {
                string dbcon = Convert.ToString(scalar).ToUpper();                
                if (dbcon == "192.168.35.19.ERPDB")
                    return ERPDBConstr;
                else if (dbcon == "192.168.35.11.FXDB")
                    return FXDBConstr;
                else if (dbcon == "192.168.35.10.TLSOFT")
                    return ZBDBConstr;
                else
                    return "";
            }
