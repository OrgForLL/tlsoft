...
public class saleTargetCore : IHttpHandler, System.Web.SessionState.IRequiresSessionState 
{
    ...
    {        
        ...
        strConn = GetDBConstrReal(Convert.ToInt32(khid)); 
        dt2 = GetSaleTarget(strConn, dtKsrq, dtJsrq, khfl, zmdfl, khid, mdmc); 
        //dt2 = GetSaleTarget(ZBDBConstr, dtKsrq, dtJsrq, khfl, zmdfl, khid, mdmc); 
        ...
        dt.Columns.Add("wcbl", typeof(decimal), "");
        dt.Columns.Add("wwce", typeof(int), "");
        dt.Columns.Add("tqzzl", typeof(decimal), "");
        
        //开始计算完成比率、未完成额、同期增长率
        foreach (DataRow dr in dt.Rows)
        {
            if (DBNull.Value.Equals(dr["xsje"]) == false && DBNull.Value.Equals(dr["mbje"]) == false && Convert.ToInt32(dr["mbje"]) > 0)
            {
            ...
            if (DBNull.Value.Equals(dr["xsje"]) == false && DBNull.Value.Equals(dr["qnje"]) == false && Convert.ToInt32(dr["qnje"]) > 0)
            {
