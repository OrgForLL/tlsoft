...
<script runat="server">
    ...
    public void GetCGData(string filterJSON) {
        ...
        using (LiLanzDALForXLM dal_dd = new LiLanzDALForXLM(DBConn)) {
            ...
            if (errinfo == "")
            {
                if (zmdfl != "" && khid == "" && mdkhid == "" && lbid == "")//第一层选择加盟或直营时获取提货数，抛弃周转量
                {
    ...
    public DataTable GetCG(string filterJSON,int cs) {
        ...
        string str_sql = @" select {0},0 - sum(b.sl * DJLB.kc) cgsl
                            ...
                            inner join yx_t_kcdjmx b on a.id=b.id                                
                            INNER JOIN t_djlxb DJLB ON a.djlx = DJLB.dm
                            INNER JOIN yx_t_khb kh ON a.tzid=kh.khid and kh.ty=0 
                            inner join yx_t_khb md on a.khid=md.khid 
                            inner join yx_t_spdmb sp on b.sphh=sp.sphh 
                            inner join yx_t_splb lb on sp.splbid=lb.id                               
        ...
        {
            case "kh":
                str_sql = string.Format(str_sql, "convert(varchar(10),a.tzid) khid,upper(kh.khdm) khdm,kh.khmc", filterStr, "a.tzid,kh.khdm,kh.khmc");
                break;
            case "md":
                str_sql = string.Format(str_sql, "a.khid,upper(kh.khdm) khdm,kh.khmc",filterStr , "a.khid,kh.khdm,kh.khmc");
                break;
            case "lb":
