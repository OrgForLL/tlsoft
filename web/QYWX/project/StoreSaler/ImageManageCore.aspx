...
<script runat="server">
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        switch (ctrl)
        {
            ...
            case "CreateImage":
                ...
                {
                    saveMyImgs(mediaid, cid, id, IsMust, tzid, username);
                }
                else
                {
    ...
    private void saveMyImgs(string mediaid, string CreateID, string id, string IsMust, string tzid, string username)
    {
        string saveUrl = "";
        ...
        string rt = "", mysql;
        DataTable dt;
        string pathStr = string.Concat("upload/ImageManage/", DateTime.Now.ToString("yyyyMM"), "/");
        using (LiLanzDALForXLM dal = new LiLanzDALForXLM(WXDBConnStr))
        {
            dt = null;
            List<SqlParameter> paras = new List<SqlParameter>();
   ...
   /**********************微信图片上传处理***************************/
    ...
    /// <param name="px"></param>
    /// <returns></returns>
    public  string DownLoadImage(string imageUrl, ref string saveImageURL)
    {
        string strInfo = "";
        lock (objDownLoad)
        {
    ...
    /// <param name="px"></param>
    /// <returns></returns>
    public string DownLoadVoice(string mediaUrl, ref string saveURL)
    {
        string strInfo = "";
        lock (objDownLoad)
        {
    ...
    /// <returns></returns>
    public string DownLoadMedia(bool IsSalerMedia, string MediaID, int MediaType, ref string saveURL)
    {
        string url;
        string access_token;
        if (IsSalerMedia)
    ...
    /// <returns></returns>
    public string UpLoadMedia(bool IsUploadToSaler, string MediaType, string MediaFile)
    {
        string url;
        string access_token;
        if (IsUploadToSaler)
