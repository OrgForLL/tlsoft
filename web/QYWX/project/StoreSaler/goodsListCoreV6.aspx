...
//        strsql = string.Format(strsql,mdid,sql_tj,sqlfilter);
        ...
        strsql = string.Concat("SELECT TOP " ,topSpCount , @" A.sphh,A.spmc,A.lsdj,A.id AS xh,ISNULL(SUM(B.sl0),0) as kc,
                ...
                WHERE A.tzid = 1 AND ISNULL(A.zp,0) = 0 AND A.kfbh NOT LIKE '2000%' {2} {3} {4}
        ...
        if (strsphhs != "")
        {
            ...
            else
            {
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    if (dr.Length > 0)
                    {
                        dt.Rows[i]["urlAddress"] = dr[0]["picUrl"];
                    }
                    else
                    {
                ...
                if (sphhList != "'zzz'")
                {
                    strsql = string.Format(@"SELECT a.sphh, MAX(b.URLAddress) URLAddress FROM yx_t_cpinfo a INNER JOIN t_uploadfile b ON a.zlmxid=b.TableID WHERE a.sphh in({0}) GROUP BY a.sphh", sphhList);
                     dt_url.Clear();
                    using (LiLanzDALForXLM MDal = new LiLanzDALForXLM(OAConnStr))
                    {
                        errinfo = MDal.ExecuteQuery(strsql, out dt_url);
                    ...
                    if (errinfo == "")
                    {
                        for (int i = 0; i < dt_url.Rows.Count; i++)
                        {
                            ...
                            dr[0]["urlAddress"] = dt_url.Rows[i]["URLAddress"].ToString();
                        }
                    }
                }
