...
<script runat="server">  
    ...
    public void  getNew_oldSale(string mdid,string ny)
    {
        string errInfo, mysql, rt = @"{{""code"":""{0}"",""sale"":{1},""msg"":""{2}""}}";
        DataTable dt_sale=null;
        using (LiLanzDALForXLM dal =new LiLanzDALForXLM(CXDBConnStr))
        {
            List<SqlParameter> paras = new List<SqlParameter>();
            mysql = @"SELECT 
                    CAST(SUM(CASE WHEN  DATEDIFF(MONTH,b.jdrq,a.rq)=0 THEN je*djlb/ABS(djlb) ELSE 0 END) AS DECIMAL(15,2)) xje,  
                    CAST(SUM(CASE WHEN  DATEDIFF(MONTH,b.jdrq,a.rq)<>0 THEN je*djlb/ABS(djlb) ELSE 0 END)AS DECIMAL(15,2)) jje   
                    FROM dbo.zmd_v_lsdjmx a 
                    INNER JOIN dbo.YX_T_Vipkh b ON a.vip=b.kh
                    WHERE  a.djbs=1 AND a.mdid=@mdid 
