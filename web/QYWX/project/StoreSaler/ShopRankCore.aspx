...
<script runat="server"> 
    ...
    public void LoadAllSale()
    {
        ...
        string khid = Convert.ToString(Request.Params["khid"]);     //如果读取的是贸易公司，则
        ...
        using (LiLanzDALForXLM ADal = new LiLanzDALForXLM(FXDBConnStr))
        {
            ...
            string strSQL = string.Concat(@"select SUM(CASE WHEN djlb > 0 THEN je ELSE 0-je END)
                                  FROM zmd_v_lsdjMX a ");
            ...
            if (khid != null && khid != "")
            {
                ...
                strSQL = string.Concat(@"DECLARE @ccid VARCHAR(50)
...
",
                    strSQL, " INNER JOIN yx_t_khb B ON A.khid=B.khid AND (A.khid = @khid OR B.ccid LIKE @ccid + '%') ");
                param.Add(new SqlParameter("@khid", khid));
            }
            else
            {
                strSQL = string.Concat(strSQL, " WHERE mdid = @mdid ");
                param.Add(new SqlParameter("@mdid", mdid));                
            }
            strSQL = string.Concat(strSQL, " AND A.djbs=1 ");
            ...
            {
                case "0":       //本月
                    SqlAdd = @" AND  A.rq >= CONVERT(CHAR(8),GETDATE(),120) + '01' 
                                AND A.rq < CONVERT(CHAR(8),DATEADD(MONTH, 1, GETDATE()),120) + '01' ";
                    break;
                case "1":       //上月
                    SqlAdd = @" AND  A.rq >= CONVERT(CHAR(8),DATEADD(MONTH, -1, GETDATE()),120) + '01' 
                                AND A.rq < CONVERT(CHAR(8),GETDATE(),120) + '01' ";
                    break;
                case "2":       //今日 
                    SqlAdd = @" AND  A.rq >= CONVERT(CHAR(10),GETDATE(),120)
                                AND A.rq < CONVERT(CHAR(10),DATEADD(DAY, 1, GETDATE()),120) ";
                    break;
                case "3":       //昨日
                    SqlAdd = @" AND  A.rq >= CONVERT(CHAR(10),DATEADD(DAY, -1, GETDATE()),120)
                                AND A.rq < CONVERT(CHAR(10),GETDATE(),120) ";
                    break;
            }
            ...
            object objSale = 0;
            string errInfo = ADal.ExecuteQueryFastSecurity(strSQL, param, out objSale);
            if (errInfo == "")
            { 
                if (objSale == null) objSale = "0";
                
                clsSharedHelper.WriteSuccessedInfo(Convert.ToString(objSale));
            }
            else
            {
                clsLocalLoger.WriteError("[琅琊榜]读取总业绩失败！错误：" + errInfo);
                clsSharedHelper.WriteErrorInfo(Convert.ToString(errInfo));                
            } 
        }
    }
    ...
    public void getRank()
    {
        ...
        //{
            ...
            if ((mdid == null || mdid == "" || mdid == "0") &&
                (khid == null || khid == "" || khid == "0") && datatype != "all")
            {
                clsSharedHelper.WriteErrorInfo("该模式下，查询不到数据~！");                
            }
            ...
            using (LiLanzDALForXLM zDal = new LiLanzDALForXLM(ZBConnStr))
            {
                using (LiLanzDALForXLM ADal = new LiLanzDALForXLM(FXDBConnStr))
                {
                    ...
                    switch (datatype)
                    {
                        ...
                        SELECT TOP 1 @GLID = ISNULL(C.ryid,0) FROM rs_T_yxrykhmx B
	                     INNER JOIN rs_t_yxrykhb C ON C.id = B.id
                                             WHERE B.mdid = @mdid AND C.djbs = 1 AND C.shbs = 1 
                        ORDER BY mxid DESC

                        IF (@GLID = 0)	SELECT ',' + CONVERT(VARCHAR(20),@mdid)
                        ELSE SELECT ',' + CONVERT(VARCHAR(20),mdid) FROM rs_T_yxrykhmx B
	                             INNER JOIN rs_t_yxrykhb C ON C.id = B.id
                             WHERE c.ryid = @GLID AND C.djbs = 1 AND C.shbs = 1  FOR XML PATH('')
                    ";
...
//                    ";  
                            ...
                            if (khid != null && khid != "")
                            {
                                strSQLKh2 = @"DECLARE @ccid VARCHAR(50), 
				                                    @khlbdm VARCHAR(10)
                    ...
                    "; 
                            ...
                            else if (mdid != null && mdid != "" && mdid != "0")//20160708 xlm 修正并简化查询方法 
                            {
                                strSQLKh2 = @"DECLARE @khid INT, 
				                                    @ccid VARCHAR(50), 
				                                    @khlbdm VARCHAR(10)
                    ...
                    ";
                            ...
                            else
                            {
                                ...
                                if (khList == "")       //如果相关客户不存在，则只查本店面
                                {
                                    ...
                                    { 
                                        strSql = string.Concat(strSql, " AND a.khid=@khid ");
                                        param.Add(new SqlParameter("@khid", khid));


                                        strSqlMe0 = string.Concat(strSqlMe0, " AND a.khid=@khid ");
                                        param2.Add(new SqlParameter("@khid", khid)); 
                                    }
                                }
                                else
                                {                                    
                    ...
                    {
                        //clsLocalLoger.WriteError("strSql=" + strSql);
