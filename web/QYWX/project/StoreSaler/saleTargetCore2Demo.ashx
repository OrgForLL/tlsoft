...
{
    private string ZBDBConstr = "server='192.168.35.10';uid=ABEASD14AD;pwd=+AuDkDew;database=tlsoft";//clsConfig.GetConfigValue("OAConnStr");
    private string AnalysisConstr = "server=192.168.35.20;database=tlsoft;uid=ABEASD14AD;pwd=+AuDkDew";//clsConfig.GetConfigValue("FXDBConStr");
    private string FXDBConstr = "server=192.168.35.11;database=fxdb;uid=ABEASD14AD;pwd=+AuDkDew";//clsConfig.GetConfigValue("FXConStr");
    private string ERPDBConstr = "server=192.168.35.19;database=erpdb;uid=ABEASD14AD;pwd=+AuDkDew";//clsConfig.GetConfigValue("ERPConStr");
    private string CX1ConStr = "server=192.168.35.20;database=CX1;uid=ABEASD14AD;pwd=+AuDkDew";//clsConfig.GetConfigValue("CX1ConStr");
    private string CX2ConStr = "server=192.168.35.20;database=CX2;uid=ABEASD14AD;pwd=+AuDkDew";//clsConfig.GetConfigValue("CX2ConStr");
    ...
    private void LoadData(HttpContext context)
    {
        if (string.IsNullOrEmpty(Convert.ToString(context.Session["qy_customersid"])))
        {
            ...
            //return;
            context.Session["qy_customersid"] = "354";
        }
    ...
    public DataTable GetSaleData(DateTime dtKsrq, DateTime dtJsrq, string khfl, string zmdfl, string khid, string mdmc)
    {
        ...
        using (LiLanzDALForXLM dal = new LiLanzDALForXLM(strConn))
        {
            ...
            lstParams.Clear();
            //添加筛选条件     
            strSQL = @"SELECT A.mdid,B.mdmc,gx.myid,
                        ...
                        INNER JOIN t_mdb B ON A.mdid = B.mdid AND A.khid = B.khid ";
            ...
            strSQL = string.Concat(strSQL, " WHERE A.djbs=1 ");
            ...
            strSQL = string.Concat(strSQL, @" GROUP BY A.mdid,B.mdmc,gx.myid ");

            //如果是总部查询（不传入khid）则根据myid进行贸易公司分组
            if (string.IsNullOrEmpty(khid))
            {
                strSQL = string.Concat(@"SELECT T.myid mdid,K.khjc mdmc,SUM(xsje) xsje FROM (", strSQL
                            , @") AS T INNER JOIN yx_t_khb K ON T.myid = K.khid 
                            WHERE K.ssid = 1 AND K.ty = 0 
                            GROUP BY T.myid,K.khjc");
            }
    ...
    public DataTable GetSaleTarget(string strConn, DateTime dtKsrq, DateTime dtJsrq, string khfl, string zmdfl, string khid, string mdmc)
    {
        ...
        using (LiLanzDALForXLM dal = new LiLanzDALForXLM(strConn))
        {
            ...
            //添加筛选条件     
            strSQL = @"SELECT A.mdid,B.mdmc,gx.myid,SUM(CONVERT(BIGINT,je)) mbje FROM yx_t_xsmbje A
            ...
            if (string.IsNullOrEmpty(khid))
            {
                strSQL = string.Concat(@"SELECT T.myid mdid,K.khjc mdmc,SUM(mbje) mbje FROM (", strSQL
                            , @") AS T INNER JOIN yx_t_khb K ON T.myid = K.khid 
                            WHERE K.ssid = 1 AND K.ty = 0 
                            GROUP BY T.myid,K.khjc");
            }
