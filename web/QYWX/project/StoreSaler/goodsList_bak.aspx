...
<script runat="server">
    string RoleName,tzid, mdid, mdmc, showType;
    ...
    <script type="text/javascript">
        ...
        function goodsList(){ 
            ...
            $.ajax({
                ...
                type: "post",
                dataType: "text",
                data: { showType:"<%=showType %>", mdid: "<%=mdid%>",tzid:"<%= tzid %>", lastID:lastID, scanResult:stxt },
                cache: false,
                timeout: 15000,
                error: function (e) {      
                    HideLoading();
                ...
                success: function (res) {  
                    if (res.indexOf("Error：") > -1) {
                        ...
                        for (var i = 0; i < obj.rows.length; i++) {                        
                            obj.rows[i]["kc"] = GetKc(obj.rows[i]["kc"]);
        ...
        function goodsListSingle(scanResult,scanType) {
            ...
            $.ajax({
                ...
                type: "post",
                dataType: "text",
                data: { showType:"<%=showType %>", mdid: "<%=mdid%>", tzid: "<%=tzid%>", scanResult: scanResult, scanType:scanType, lastID:searchLastID },
                cache: false,
                timeout: 15000,
                error: function (e) {				    
                    HideLoading();
        ...
        function goodsStock(scanResult,scanType){
            $.ajax({
                ...
                type:"post",
                dataType: "text",
                data: { showType:<%=showType %>,mdid:<%=mdid %>,tzid:"<%= tzid %>",scanResult:scanResult,scanType:scanType},
                catch: false,
                timeout: 15000,
                error: function(e){
                    HideLoading();				
                ...
                success:function (res){
                    if(res.indexOf("Error")>-1){
                        ...
                        for(var i=0;i<obj.rows.length ;i++){
                            if(obj.rows[i]["sl"]>0){                            
                                obj.rows[i]["sl"] = GetKc(obj.rows[i]["sl"]);
