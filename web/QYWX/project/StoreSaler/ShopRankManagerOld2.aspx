...
<script runat="server">
    ...
    protected void Page_PreRender(object sender, EventArgs e)
    {
        ...
        {
            string strSQL = string.Concat(@"SELECT A.khid,A.khmc mdmc,A.ssid FROM yx_t_khb A 
    ...
    private void CalKhlist(ref DataTable dt)
    {
        if (dt != null)
        {
            string optionBase = @"<option value=""{0}|{1}"">{2}</option>";
            StringBuilder sbOption = new StringBuilder();
            foreach (DataRow dr in dt.Rows)
            {
                sbOption.AppendFormat(optionBase, dr["khid"], dr["ssid"], dr["mdmc"]);
            }
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <div class="header">
        <span class="nav-item2 current-nav" onclick="changeArea('province')" id="province">
            <span id="spkhsel" onclick="ShowSelect();">请选择<i class="fa fa-caret-down fa-lg"></i></span>            
            <select id="khsel">  
              <%= khOptions %>
    ...
    <script type="text/javascript">
        ...
        function getRank(divname, rankname, datamonth) {
            ...
            if (divname == "province") {    //如果是按省份（贸易公司）
                ...
                if (typeof (selectVal) == undefined || selectVal == "") { showLoader("error", "必须选择一个贸易公司！请联系IT部。"); return; }
                else {
                    var selval = selectVal.split("|");

                    khid = selval[0];
                    if (selval[1] == "1") interfaceDivName = "province";
                    else interfaceDivName = "shop";                         
        ...
        function changeArea(objid, datamonth) { 
            ...
            var khmc = $("#khsel").find("option:selected").text();
            $("#spkhsel").html(khmc + "<i class='fa fa-caret-down fa-lg'></i>");
