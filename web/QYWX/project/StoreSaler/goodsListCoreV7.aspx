...
<script runat="server">  	   
    ...
    string queryConnStr = "server=192.168.35.20;uid=ABEASD14AD;pwd=+AuDkDew;database=tlsoft";
    ...
    private const string erpLink = "http://webt.lilang.com:9001";      //对于服务器而言的主应用层链接URL 
    ...
    private void SaveEvaluation(string paraStr)
    {
        ...
        if (tzid == null || tzid == "" || tzid == "0") tzid = "1";
        if (mdid == null || mdid == "" || mdid == "0") mdid = "1";
        ...
        if (prarJson.GetJsonValue("ID") == "" || prarJson.GetJsonValue("ID") == "0")
        {
            ...
            para.Add(new SqlParameter( "@khid",tzid));
    ...
    public string LoadGoodsStockAuto(string khid, string sphh)
    {
        ...
        string strSql;
        DataTable dt1, dt2, dt3, dt4, dt4_2 = null, dt5 = null, dt6 = null;
        string strInfo = "";
        ...
        using (LiLanzDALForXLM mydal = new LiLanzDALForXLM(OAConnStr))
        {
            ...
            //首先确定上级khid是不是总部，如果是则查主库，否则查对应库 
            khConnString = getDBConn(khid);

            strSql = "SELECT TOP 1 ssid FROM yx_t_khb WHERE khid=@khid";
            lstSqlParam.Clear();
            lstSqlParam.Add(new SqlParameter("@khid", khid));
            object objSsid = null;
            strInfo = mydal.ExecuteQueryFastSecurity(strSql, lstSqlParam, out objSsid);
            if (strInfo != "" || objSsid == null) { clsLocalLoger.WriteError(string.Concat("读取客户ssid信息失败！错误：", strInfo)); return ""; }

            if (Convert.ToInt32(objSsid) == 1) mydal.ConnectionString = queryConnStr;
            else mydal.ConnectionString = khConnString;
            ...
            {
                strSql = @"
                    DECLARE @qykhbs VARCHAR(20) 

                    SELECT @qykhbs = ''  

                    SELECT TOP 1 @qykhbs = qykhbs FROM yx_t_khb WHERE khid = @khid

                    IF (@qykhbs <> '')  
                    BEGIN  
	                    SELECT A.cmdm,SUM(A.sl0) 共享库存 FROM yx_t_spkccmmx A
	                    INNER JOIN (SELECT khid,khmc FROM yx_t_khb WHERE qykhbs = @qykhbs AND khid <> @khid) B ON A.tzid = B.khid 
                        WHERE A.sphh= @sphh 
		                    GROUP BY A.cmdm 
                    END ";
                ...
                //clsLocalLoger.WriteInfo("调试 khid=" + khid);
                //clsLocalLoger.WriteInfo("sphh=" + sphh);

                strInfo = mydal.ExecuteQuerySecurity(strSql, lstSqlParam, out dt4_2);

                if (strInfo != "") { clsLocalLoger.WriteError(string.Concat("读取共享库存信息失败！错误：", strInfo)); return ""; }

                if (dt4_2 != null && dt4_2.Rows.Count == 0)
                {
                    dt4_2.Clear(); dt4_2.Dispose(); dt4_2 = null;
                }
            }
            ...
            //共享库存字段处理
            if (dt1.Columns.Contains("共享库存"))
            {
                dt1.Columns.Add("库存(共享)", typeof(string));
                foreach (DataRow dr in dt1.Rows)
                {
                    dr["库存(共享)"] = string.Concat(dr["库存"], "(", dr["共享库存"], ")");
                }

                dt1.Columns.Remove(dt1.Columns["库存"]);
                dt1.Columns.Remove(dt1.Columns["共享库存"]);
                n--;
            }
    ...
    private string getDBConn(string tzid)
    {
        ...
        {
            string DBName;
            if (tzid == "1") DBName = "TLSOFT";
            else DBName = dal.GetDBName(Convert.ToInt32(tzid)).ToUpper();

            switch(DBName){
                case "FXDB":
                    connecting = FXDBConStr;break;
                case "ERPDB":
                    connecting = ERPDBConStr;break;
                case "TLSOFT":
                    connecting = OAConnStr;break;
                case "ESOFT":
                    string ESoftConnStr = clsConfig.GetConfigValue("ESoftConStr");
                    connecting = ESoftConnStr;break;
                default:
                    clsLocalLoger.WriteError(string.Format("无法获取套帐数据库！tzid={0}", tzid));
                    connecting = "";
                    break;
            }
        }
    ...
    public string LoadCmStock(string khid, string sphh, string cm)
    {
        ...
        using (LiLanzDALForXLM mydal = new LiLanzDALForXLM(OAConnStr))
        {
            if (RoleName == "dg" || RoleName == "dz")//导购店长的门店ID换取khid
            {
                ...
                using (LiLanzDALForXLM dal = new LiLanzDALForXLM(OAConnStr))
                {
                    strInfo = dal.ExecuteQueryFast(string.Format("select khid from t_mdb where mdid={0}", khid), out objtzid);
                    if (strInfo == "")
                    {
                        khid = objtzid.ToString();
                    }
                }
            }
            ...
            strSql = @"DECLARE @cmdm VARCHAR(20)
                        ...
                        WHERE A.sphh= @sphh AND A.cmdm = @cmdm 
                         AND (A.tzid = @khid OR B.ccid LIKE @ccid)
            ...
            dt1.Clear(); dt1.Dispose(); lstInfo.Clear();
            ...
            if (RoleName == "dg" || RoleName == "dz")//导购店长的门店ID换取khid
            {
                ...
                strSql = @"
                    ...
                    IF (@qykhbs <> '')  
                    BEGIN  
	                    SELECT B.khmc,C.mc ckmc,A.sl0 sl FROM yx_t_spkccmmx A
	                    INNER JOIN (SELECT khid,khmc FROM yx_t_khb WHERE qykhbs = @qykhbs AND khid <> @khid) B ON A.tzid = B.khid
                      INNER JOIN YX_T_Ckdmb C ON A.ckid = C.id
                      WHERE A.sphh= @sphh AND A.cmdm = @cmdm
		                    ORDER BY A.sl0 DESC
                ...
                if (strInfo != "") { clsLocalLoger.WriteError(string.Concat("读取商品指定尺码共库存信息失败！错误：", strInfo));  }
    ...
    public string goodsList(string mdid, string lastId, string filter)
    {
        ...
        if (RoleName == "dg" || RoleName == "dz")//导购店长的门店ID换取khid
        {
            ...
            using (LiLanzDALForXLM dal = new LiLanzDALForXLM(OAConnStr))
            {
                //clsLocalLoger.WriteInfo(string.Concat("连接字符串1：" + dal.ConnectionString));
        ...
        if (lastId != "-1")
        {
            sql_tj = string.Format(@"and   (CASE WHEN spmc LIKE '%高端%' THEN 8 
                      WHEN spmc LIKE '%西服%' THEN 7
                      WHEN spmc LIKE '%正%衬%' THEN 6
                      WHEN spmc LIKE '%休%衬%' THEN 5          
                      WHEN spmc LIKE '%鞋%' THEN 3                         
                      WHEN spmc LIKE '%配饰%' THEN 2
                      ELSE 4 END)*1000000+ A.id<{0}", lastId);
        }
        ...
        if (!string.IsNullOrEmpty(filter))
        {
            ...
            foreach (string str in filterArry)
            {
                ...
                switch (filterOne[0])
                {
                    ...
                    case "kfbh":
                        {
                            otherFilter = string.Concat(otherFilter, " AND A.kfbh= '", filterOne[1], "' ");
                        }
                        break;
                    case "fl":
                        {
                            otherFilter = string.Concat(otherFilter, " AND A.splbid= ", filterOne[1], " ");
                        }
                        break;
                }
            }
        ...
        /* strsql = string.Concat("SELECT TOP " ,topSpCount , @" A.sphh,A.spmc,A.lsdj,A.id AS xh,ISNULL(SUM(B.sl0),0) as kc,
                 ...
                 WHERE A.tzid = 1 AND ISNULL(A.zp,0) = 0 AND A.kfbh NOT LIKE '2000%' AND kfbh <='", getKfbhOpen(), @"' {2} {3} {4} {5}
                 ...
                 (CASE WHEN spmc LIKE '%高端%' THEN 6 
                       ...
                       ELSE 0 END) DESC
                 ,A.ID DESC");*/
        strsql = string.Concat("SELECT TOP " ,topSpCount , @" A.sphh,A.spmc,A.lsdj,ISNULL(SUM(B.sl0),0) as kc,'' as urlAddress,
                    (CASE WHEN spmc LIKE '%高端%' THEN 8 
                      WHEN spmc LIKE '%西服%' THEN 7
                      WHEN spmc LIKE '%正%衬%' THEN 6
                      WHEN spmc LIKE '%休%衬%' THEN 5          
                      WHEN spmc LIKE '%鞋%' THEN 3                         
                      WHEN spmc LIKE '%配饰%' THEN 2
                      ELSE 4 END)*1000000+a.id AS xh
                FROM YX_T_Spdmb A
                LEFT JOIN yx_t_spkccmmx B ON A.sphh = B.sphh AND B.tzid = {0}
                WHERE A.tzid = 1 AND ISNULL(A.zp,0) = 0 AND A.kfbh NOT LIKE '2000%' AND kfbh <='", getKfbhOpen(), @"' {2} {3} {4} {5}
                GROUP BY A.sphh,A.spmc,A.lsdj,A.id
                {1}
                ORDER BY  xh desc");
        strsql = string.Format(strsql, tzid, KcFilter , sql_tj, SpFilter, ZtFilter, otherFilter);
        ...
        using (LiLanzDALForXLM dal = new LiLanzDALForXLM(ConString))
        {
            ...
            //}
            //clsLocalLoger.WriteInfo(string.Concat("连接字符串2：" + dal.ConnectionString));
            ConString = getDBConn(tzid);
            dal.ConnectionString = ConString;

            //clsLocalLoger.WriteInfo(string.Concat("连接字符串3：" + dal.ConnectionString));
    ...
    public string goodsDetail(string sphh,string showType,string mdid)
    {
        ...
        strsql = string.Format(@"
                               ...
                               isnull(b.urladdress,'') qsreport
						       from yx_T_spdmb sp 
                               left join (
                                    SELECT top 1 b.urladdress 
                                    FROM dbo.YX_T_Spdmb a 
                                    INNER JOIN t_uploadfile b ON a.id=b.TableID
                                    WHERE a.sphh='{0}' AND b.GroupID=2336
                                    ORDER BY b.id desc
                                ) b on 1=1
    ...
    private string otherDetail(string mdid, string sphh)
    {
        ...
        temp = SellingPointImg(sphh);
        ogdJson.AddJsonVar("SellingPointImg", temp);
        rt = ogdJson.jSon;
        return rt;
    }
    public string LoadCMInfos(string sphh)
    {
        ...
        using (LiLanzDALForXLM dal = new LiLanzDALForXLM(OAConnStr))
        {
            string str_sql = @"declare @sql varchar(8000);declare @tsql varchar(8000);declare @sid int;
                                ...
                                select @sql=' select cm.cm ''尺码'','+@tsql+' from yf_t_ytfamxb a inner join yx_t_cmzh cm on cm.cmdm=a.dm where a.id='+convert(varchar,@sid)+' and cm.tml='''+isnull((select tml from yx_t_spdmb where sphh=@sphh),'')+''' group by cm.xm,cm.cm order by cm.xm,len(cm.cm)';
                                exec(@sql);
                                end";
            List<SqlParameter> paras = new List<SqlParameter>();
    ...
    //卖点图
    public string SellingPointImg(string sphh)
    {
        string errorInfo;
        List<string> strList = new List<string>();
        string strsql = @" 	select urlAddress
                            From yx_t_spdmb a 
                            inner join t_uploadfile b on a.id=b.tableid and b.groupid=16264 
                            where a.sphh=@sphh ";
        ...
        if (errorInfo != "")
        {
            clsLocalLoger.WriteError("[商品库存查询]读取卖点图片失败！错误：" + errorInfo);
            return "";
        }
        else
        {
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                strList.Add(string.Concat(erpOutLink , Convert.ToString(dt.Rows[i][0]).Replace("../","/")));
            }
        }
    ...
    private void LoadReport(string sphh)
    {
        string errInfo,rt = "";
        object objZzbgUrl = "";
        using (LiLanzDALForXLM dal = new LiLanzDALForXLM(OAConnStr))
        {
            errInfo = dal.ExecuteQueryFast(string.Format("EXEC yx_cx_sphh_jybg '{0}'", sphh), out objZzbgUrl);
        }

        if (errInfo != "")
        {
            clsLocalLoger.WriteError(string.Concat("[goodsListCoreV7.aspx-LoadReport]SQL执行失败！错误：", errInfo));
            clsSharedHelper.WriteErrorInfo("系统繁忙！");
            return;
        }

        if (Convert.ToString(objZzbgUrl) == "")
        {
            clsSharedHelper.WriteErrorInfo("暂无质检报告！");
            return;
        }

        clsSharedHelper.WriteInfo(objZzbgUrl.ToString());
    }
