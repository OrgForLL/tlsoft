...
public class CoffeePayback : IHttpHandler
{
    ...
    public void ProcessRequest (HttpContext context) {
        ...
        {            
            ...
            content = pay.Decrypt(content);
            payBack(content); 
            //clsLocalLoger.WriteInfo("content:" + content);
    ...
    public void GetCoffeeOrderList(int CoffeeMdid, string terminalNo, string Finddate,string status,string bindstatus)
    {
        ...
        if (status == "1") sb.Append(" AND status = 1 ");
        else if (status == "2") sb.Append(" AND status <> 1 ");
    ...
    {
        clsLocalLoger.WriteInfo("[银联支付]检查支付结果：" + xml);        
        ...
        if (result.SelectSingleNode("root/head/result/code").InnerText == "00")
        {
            ...
            using (LiLanzDALForXLM dal = new LiLanzDALForXLM(dbConn))
            {
                ...
                if (backbody.SelectSingleNode("status") != null && backbody.SelectSingleNode("status").InnerText == "0")        //增加关键判断 20170622 . by:xlm
                {
                    clsSharedHelper.WriteSuccessedInfo("请根据支付设备提示进行操作！");
                    return;                                            
                }
                ...
                { //收款行为
                    ...
                    else clsSharedHelper.WriteSuccessedInfo("支付失败！");
                    return;
                }
    ...
    {
        clsLocalLoger.WriteInfo(string.Format("[银联支付]发出检查请求。terminalNo={0} , ordersn={1}", terminalNo, ordersn));
        UnionPay pay = new UnionPay();
        pay.InnerMerchantNo = terminalNo;
        //string xml = pay.query(terminalNo, ordersn.ToString(), "1");
        string xml = query(terminalNo, ordersn.ToString(), "1");
        
        CheckBindBillOrderNo(ordersn);         
        
        payBack(xml);//重新回填信息 
        return "ok";
    }
    
    public void CheckBindBillOrderNo(Int64 ordersn){ 
        string oaConn = clsConfig.GetConfigValue("OAConnStr");

        string strInfo = "";
        using (LiLanzDALForXLM zbdal = new LiLanzDALForXLM(oaConn))
        {
            object payid = null;
            strInfo = zbdal.ExecuteQueryFast(string.Concat("SELECT TOP 1 ID FROM cy_t_BillPayInfo WHERE PayID = '" , ordersn.ToString(),"'"), out payid);
            if (strInfo != "")
            {
                clsLocalLoger.WriteError("[咖啡馆]支付状态检查失败！错误：" + strInfo);
                return;
            }

            
            int BindBillOrderNo = 0;
            if (payid != null)
            {
                BindBillOrderNo = 1;                
            }
            ...
            string dbConn = getDBConn();
            using (LiLanzDALForXLM wxdal = new LiLanzDALForXLM(dbConn))
            {
                strInfo = wxdal.ExecuteNonQuery(string.Format("UPDATE cy_t_UnionpayOrder SET BindBillOrderNo = {0} WHERE orderNo = {1}" , BindBillOrderNo,ordersn));
                if (strInfo != "")
                {
                    clsLocalLoger.WriteError("[咖啡馆]更新支付状态失败！错误：" + strInfo);
                    return;
                }
            }
        }
    ...
    public string query(string terminalNo, string orderNo, string type)
    {
        long dt = DateTimeToStamp(DateTime.Now);

        String appid = UnionPayConfig.appid;//平台分配的appid
        String appkey = UnionPayConfig.appkey;//平台分配的appkey
        String url = UnionPayConfig.url;

        XmlDocument docheader = new XmlDocument();
        XmlElement header = docheader.CreateElement("head");
        header.AppendChild(docheader.CreateElement("type")).InnerText = "D007";
        header.AppendChild(docheader.CreateElement("version")).InnerText = "1";
        header.AppendChild(docheader.CreateElement("time")).InnerText = dt.ToString();
        header.AppendChild(docheader.CreateElement("appid")).InnerText = UnionPayConfig.appid;

        XmlElement body = docheader.CreateElement("body");
        //body.AppendChild(docheader.CreateElement("innerMerchantNo")).InnerText = InnerMerchantNo;//"161201100003"; //内部商户号
        body.AppendChild(docheader.CreateElement("terminalNo")).InnerText = terminalNo;//终端号
        body.AppendChild(docheader.CreateElement("orderNo")).InnerText = orderNo;//订单号
        body.AppendChild(docheader.CreateElement("payTransType")).InnerText = type;

        StringBuilder sb = new StringBuilder();
        sb.Append(appkey).Append("|").Append(header.OuterXml).Append("|").Append(body.OuterXml);
        String sign = Security.MD5Hash(sb.ToString());
        sb.Length = 0;
        sb.Append("<?xml version='1.0' encoding='UTF-8'?>");
        sb.Append("<root>");
        sb.Append(header.OuterXml);
        sb.Append(body.OuterXml);
        sb.Append("<sign>");
        sb.Append(sign.ToLower());
        sb.Append("</sign>");
        sb.Append("</root>");
        LogHelper.Info(sb.ToString());
        String req = Security.DES3Encrypt(sb.ToString(), UnionPayConfig.MESSAGE_3DES_KEY);
        HttpHelper http = new HttpHelper();                    
        String back = http.PostTimeOut(url, 6000, req);         
        back = Security.DES3Decrypt(back, UnionPayConfig.MESSAGE_3DES_KEY);
        LogHelper.Info(back);
        return back;
    }
