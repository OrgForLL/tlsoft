...
<script runat="server">
    ...
    private void init()
    {
        ...
        if (vipkh == "15708894544" || vipkh == "18687973437" )
        {
            res = ResponseModel.setRes(401, "", "系统检测到你存在恶意改单行为,不允许再参与本活动!(我们保留追索奖品的权利)");
            clsSharedHelper.WriteInfo(JsonConvert.SerializeObject(res));
            return;
        }
                ...
                if (PrizeIndex < PrizeCount)
                {
                    ...
                    if (prizeid == superLuck)//中特等奖，判定该省份能不能中奖、
                    {
                        ...
                        else
                        {
                            ...
                            {
                                int maxValue = enlarge;// +24 - DateTime.Now.Hour;    //时间越晚，中奖率概率越大；由于最后三个省份开单量较少，为了提高中奖率只好降低放大难度
