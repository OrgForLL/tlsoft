...
<script runat="server"> 
    ...
    private const string ConfigKeyValue = "2";
       ...
       if (errInfo != "")
       {
        ...
        else if (OpenId != "")//未关注人员，openid不为空
        {
            mySql =@"select a.name,a.cname,a.mobile,a.status,a.id,a.status,isnull(b.userid,0) authID 
                    ...
                    left join wx_t_AppAuthorized b on a.id=b.userid and b.systemid=@systemid where a.wxopenid=@wxopenid";
            para.Add(new SqlParameter("@wxopenid", OpenId));
            para.Add(new SqlParameter("@systemid", ConfigKeyValue));
            ...
            else if (dt.Rows.Count < 1)//此openid不存在，说明此人不存在企业通讯录
            {
                ...
                Response.Redirect("../BandToSystem/SystemBand.aspx?systemid=" + ConfigKeyValue + "&ClosePag=1");
            }
            else
            {
        ...
        else //已关注成功
        {
            ...
            {
                outputjs = GetAlertString("您尚未绑定人资系统", "操作提示");
                Response.Redirect("../BandToSystem/SystemBand.aspx?systemid=" + ConfigKeyValue + "&ClosePag=1");
            }
    ...
    public string CheckQrCodeLogin(string uuid, string wxopenid)
    {
        ...
        string exeSQL = @"DECLARE @userid INT;
                            ...
                            INNER JOIN wx_t_AppAuthorized b ON a.id=b.userid AND b.systemid=@systemid AND a.name=@wxOpenid
                            IF (@userid = 0)	SELECT '0' AS Cstate,'Type4|该微信号还未绑定人资系统，无法登录！' AS msgInfo
                            ELSE
                            BEGIN
        ...
        listSqlParameter.Add(new SqlParameter("@systemid", ConfigKeyValue));
