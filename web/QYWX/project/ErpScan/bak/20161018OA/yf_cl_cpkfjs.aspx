...
<head runat="server">
    ...
    <script type="text/javascript">    
        var appIdVal, timestampVal, nonceStrVal, signatureVal, userssid, userid, username,scanCtrl;
        var bcbs = "1",fzid="0",username="";
        ...
        */
                        ...
                        $.prompt(showHtml,
                        {
                            ...
                            submit: function (e, v, m, f) {
                                ...
                                if (v == "iscancel") {
                                    ...
                                    ajaxSubmit(checkInfo.key, checkInfo.yphh, bcbs, fzid,username); //一定是同步ajax
                                }
                            },
        ...
        //ajax提交
        function ajaxSubmit(key, yphh,bcbs,fzid,username) {
            var subData = { key: key, bcbs: bcbs, fzid: fzid, yphh: yphh, userid: document.getElementById("useridVal").value, username: username };
        ...
        function getShowHtml(objJson) {
            ...
            if (scanCtrl == "ypcjjs") {
                ...
                showHtml += "</br></br><label style='font-size:20px;'><input name='moban'style='width:20px;height:20px;' checked id='radiogr' type='radio' onclick='bcbs=1' value='' />个人</label>"
                showHtml += "<label style='font-size:20px;'><input style='width:20px;height:20px;' name='moban' id='radiogr' type='radio' onclick='bcbs=2,fzid=2' value='' />裁剪组</label>";
                if (objJson.cjjsbs == 1) {
                ...
                showHtml += "</br></br><label style='font-size:20px;'><input name='moban' style='width:20px;height:20px;' id='wgqrgr' type='radio' checked onclick='bcbs=1' value='' />个人</label>";
                for (i = 0; i < objJson.num ; i++) {
                    var fz = objJson.fz.split(";");
                    showHtml += "<label style='font-size:20px;'><input name='moban' style='width:20px;height:20px;' id='wgaz' type='radio' onclick='wgqraz()' value='" + fz[i] + "' />" + fz[i] + "</label>"
                   }
                if (objJson.zyjsbs == 1) {
        ...
        function wgqraz() {
            bcbs = 2;
            //fzid = document.getElementById("wgaz");
            //alert(fzid);
            var obj = document.getElementsByName("moban");
            for(var i=0; i<obj.length; i ++){
                if(obj[i].checked){
                    username=obj[i].value;
                }
            }
        ...
        function getBtnJson(objJson) {
            ...
            else if (objJson.zyjsbs == 0 && scanCtrl == "ypwgqr" && objJson.cjjsbs == 1) {
                buts.确认 = "sub";
            } 
            buts.取消 = "iscancel";
            return buts;
