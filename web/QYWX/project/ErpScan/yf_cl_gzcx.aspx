    ...
    <script type="text/javascript">    
        ...
        $(document).ready(function () {
            ...
            var scroll = new auiScroll({
                ...
                if (ret.isToBottom) {
                    ...
                    var dqny = $("#ny").val();
                    if (dqny.length == 5) {
                        dqny = dqny.substring(0, 4) + '0' + dqny.substring(dqny.length - 1, dqny.length);
                    } 
                    if (getdata) {
        ...
        function setWage(msg) {
            if (msg.indexOf("Error") == -1 && msg != "") {
                ...
                //StaticCount("yggz");
                try{
                    $("#yggz [col='s-sl']").text(datas.rows[0].zsl);
                    $("#yggz [col='s-je']").text(datas.rows[0].zje);
                } catch (e) { }
                HideLoading();
                var nr = "";
        ...
        function StaticCount(id) {
            ...
            for (var i = 0; i < ulobj.length; i++) {
                var je = $("p[col='je']").eq(i).text();
                var sl = $("p[col='sl']").eq(i).text();
                if (je == "")
                    je = "0";
                if (sl == "")
                    sl = "0";
                s_sl += parseFloat(sl);  //此处要转换成浮点型，否则对于小数会被四舍五入掉
                s_je += parseFloat(je);
            }//end for
            ...
            <div class="static-title">
                <ul class="floatfix" >
                    <li class="item-head fixed">
                        <p class="data-item col4"hidden>编号</p>
                        <p class="data-item col4">接收日期</p>
                        <p class="data-item col15">加工价</p>
                        <p class="data-item col15">数量</p>
                        <p class="data-item col15">金额</p>
                        <p class="data-item col15"  hidden>商品名称</p>
                        <p class="data-item col15">产品名称</p>
                        <p class="data-item col15">样品货号</p>
                        <p class="data-item col15">裁剪方式</p>
                        <p class="data-item col15">工艺等级</p>
                        <p class="data-item col15">款式难度</p>
                    </li>
                </ul>
            </div>
