...
<style>   
    ...
    .h1 {
    font-size:30px;color:black;text-align:center;font-family:initial;font-weight:900;margin-top:20px;margin-bottom:30px;
    }
    .ui-grid-a{
    margin-left: auto;
margin-right: auto;
...
<script language="vb" runat="server">
    Dim menuLb, menuMb, menuBt, MySql, bHtml, errinfo, aHtml As String
    ...
    Sub page_load(ByVal S As Object, ByVal E As EventArgs)
        ...
        menuBt = Trim(Request.Params("menuBt"))
        If (menuBt <> "") Then
            aHtml += "<h1 class='h1' >" + menuBt.ToString() + "</h1>"

        Else
            aHtml += "<h1 class='h1' >样衣车间刷码</h1>"
        End If
    ...
    Sub getData(ByVal menulb As String, ByVal menumb As String)
        ...
        MySql += " select b.id,b.m_name,b.m_asp,b.m_bz from t_menu a inner join t_menu b on a.id=b.ssid where a.ssid=@menuid and b.m_pass=@menumb and b.m_onoff=1 order by b.m_dm;"
    ...
    Function getHtml(ByVal dt As DataTable) As String
        ...
        For Each MyDr As DataRow In dt.Rows
            ...
            If i Mod 2 = 1 Then
                bHtml += "<a href = '" + m_asp + "' target='_self' Class='weui_btn weui_btn_primary'>" + MyDr("m_name").ToString() + "</a>"
            Else
                bHtml += "<a href = '" + m_asp + "' target='_self' Class='weui_btn weui_btn_primary'>" + MyDr("m_name").ToString() + "</a>"
            End If
        Next
