...
<script runat="server">   
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (url.IndexOf("&tlbt") > 0)
        {
            ...
            //Response.Redirect(newUrl + "&userid=12742" );
            Response.End();
        }
        else
        {
                ...
                try
                {
                    ...
                    using (LiLanzDALForXLM dal = new LiLanzDALForXLM(OAConnStr))
                    {
                        ...
                        string str_sql = @"
                         ...
                         select  case when t.mc is null then 'forbid' else 'allow' end useridCondition, sq.mxid sqmxid,sq.sl as sqsl,sq.zdr,a.yphh as mlbh,a.chdm as llyphh,sp.mc spmc,sq.mxbz,xl.mc as xlmc ,case isnull(b.id,0) when 0 then 0 else 1 end isjs
                         from cl_v_dddjmx a  
                         ...
                         left join (
                             ...
                             inner JOIN dbo.t_user d ON d.rtx=c.bz
                             WHERE  a.djlx1=9208 and a.ty<>1 AND  a.mc='项目经理-设计师' and isnull(c.bz,'')<>''  and d.id= @userid    
                             union
            ...
            wx.scanQRCode({
                ...
                success: function (res) {
                    ...
                    if (checkInfo.result == "Successed") {
                        $.prompt("<div style='font-size:15px;'>材料编号:" + checkInfo.chdm + "</br>材料名称:" + checkInfo.chmc + "</br>申请人:" + checkInfo.zdr + (checkInfo.useridCondition == "forbid" ? "(非申请人不能领用)" : "") + "</br>调样米数:" + checkInfo.sqsl + "</br>领用状态:" + (checkInfo.isjs == 1 ? "已领用" : "未领用</br>请选择是否领用,如果不想操作请选择[取消]") + "</div>",
                        {
                            title: "提示",
                            buttons: (checkInfo.isjs == 1 || checkInfo.useridCondition == "forbid" ? { '取消': 'iscancel'} : { "领用": "sub", '取消': 'iscancel' }),
