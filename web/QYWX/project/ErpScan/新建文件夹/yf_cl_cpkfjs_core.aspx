...
<script runat="server">  
            ...
            using (LiLanzDALForXLM dal = new LiLanzDALForXLM(OAConnStr))
            {
                ...
                if (cllx == "cjjs")
                {
                    str_sql = @"
                         ...
                         begin
                            ...
                            begin
                                declare @ryid int;declare @rymc varchar(15);declare @bl decimal(8,2);
                                select @ryid=b.ryid,@rymc=b.rymc,@bl=b.bl from (select ROW_NUMBER()over(order by a.ryid ) as xh,a.rymc,a.ryid,a.bl From yf_v_cpkfmbz a where a.fzid=@fzid)b where b.xh=@j ;
                                Insert into yf_t_yprygxb (tzid,id,mxid,djlx,dxlx,val,txt,bl) values(@tzid,@key,0,'tgry','cjs',@ryid,@rymc,@bl);
                                set @j=@j+1
                            end
                ...
                }else if (cllx == "cjfl")
                {
                    str_sql = @"declare @username varchar(20);declare @xmjl varchar(20);declare @bs varchar(20);declare @yygid varchar(20);set @username='';
                         ...
                         set @j=1;
                         select @username=xm from rs_t_ryxxb where id=@userid;
                         select @yygid=id from rs_t_ryxxb where xm=@user;
                         ...
                         begin
                            if @user = 'T恤组' 
                                begin
                                    select @i=count(a.rymc) from yf_v_cpkfmbz a where a.fzmc='样衣组' ;
                                    select @userss=stuff((select ';'+a.rymc from yf_v_cpkfmbz a where a.fzmc='样衣组' for xml path('')),1,1,'');
                                    delete from yf_t_yprygxb where tzid=@tzid and id=@key and djlx='tgry' and dxlx='yygs';
                                    while @j<@i+1 and @j<11
                                        begin
                                            declare @ryid int;declare @rymc varchar(15);declare @bl decimal(8,2);
                                            select @ryid=b.ryid,@rymc=b.rymc,@bl=b.bl from (select ROW_NUMBER()over(order by a.ryid ) as xh,a.rymc,a.ryid,a.bl From yf_v_cpkfmbz a where a.fzmc='样衣组')b where b.xh=@j ;
                                            Insert into yf_t_yprygxb (tzid,id,mxid,djlx,dxlx,val,txt,bl) values(@tzid,@key,0,'tgry','yygs',@ryid,@rymc,@bl);
                                            set @j=@j+1
                                        end
                                ...
                                begin
                                    delete from yf_t_yprygxb where tzid=@tzid and id=@key and djlx='tgry' and dxlx='yygs';
                                    update yf_t_cpkfsjtg set yyg=@user+';',cjflbs=1,cjflr=@username,cjfsrq=getdate(),xmjl=@xmjl,xmjlid=@xmjlid,bs=@bs,bsid=@bsid where id=@key;
                                    insert into yf_t_yprygxb (tzid,id,mxid,djlx,dxlx,val,txt,bl) values(@tzid,@key,0,'tgry','yygs',@yygid,@user,'100');
                                    insert into xt_t_djshjl (tzid,zblx,zbid,xh,shgwid,shsj,shr,shzt,shyj,dadm) values(@tzid,1005,@key,0,0,getdate(),@username,1,'手机裁剪发料','');                         
                                end
                         end else if @bcbs ='1'
                         begin
                ...
                {
                    str_sql = @"
                          declare @username varchar(20);declare @xmjl varchar(20);declare @bs varchar(20);declare @yygid varchar(20);set @username='';
                         declare @i int;declare @j int;declare @userss varchar(200);
                         select @username=xm from rs_t_ryxxb where id=@userid;
                         begin
                            update yf_t_cpkfsjtg set zyjsr=@username,zyjsrq=getdate(),zyjsbs=1,xmjl=@xmjl,xmjlid=@xmjlid,bs=@bs,bsid=@bsid where id=@key;
                            ...
                            select 11228 as tzid,a.ypbh as sphh,'' shdm,e.ypkh spkh,e.ypmc spmc,a.ypbh as yphh,c.kfbh from yf_T_cpkfzlb a 
    ...
    public void GetInfo( string cxlx)
    {
        try
        {
            ...
            {
                string str_sql = @"
                   select top 1 a.id,a.cjs,a.yyg,a.flowid,a.jsr,c.kfbh,c.xlid,fg.mc as spfg,a.yphh,b.ypzlbh,e.mc as splbmc,isnull(a.cjjsbs,0) as jsbs,
