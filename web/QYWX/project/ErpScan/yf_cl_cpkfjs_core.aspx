            ...
            using (LiLanzDALForXLM dal = new LiLanzDALForXLM(OAConnStr))
            {
                ...
                }else if (cllx == "cjfl")
                {
                    str_sql = @"declare @username varchar(20);declare @xmjl varchar(20);declare @bs varchar(20);declare @yygid varchar(20);set @username='';
                         ...
                         begin
                                if @fzmc = 'T恤组' or @fzmc='休衬组' or @fzmc='正衬组' or @fzmc='轻商务组' or @fzmc='短裤组' or @fzmc='内衣组'
                                begin
                                    select @userss=(select a.rymc+';' from yf_v_cpkfmbz a where a.fzmc=@fzmc for xml path(''));
                                    delete from yf_t_yprygxb where tzid=@tzid and id=@key and djlx='tgry' and dxlx='yygs';
        ...
        try
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                if (cxlx == "wgqr"||cxlx=="cjfl") {                    
                    using (LiLanzDALForXLM MyDal = new LiLanzDALForXLM(OAConnStr))
                    {
                        string MySql = @"
                             ...
                             declare @zdxmjl varchar(20);declare @ryid int;declare @chmc varchar(200);declare @cfbl varchar(200);declare @ghsmc varchar(200);declare @dj varchar(200);
                             select top 1 @chmc=a.mlbh,@cfbl=b.cfbl,@ghsmc=kh.khmc,@dj=cast(b.cbdj as varchar) from yf_t_cpkfplxx_ml a inner join cl_t_chdmb b on a.chdm=b.chdm 
                                  inner join yx_t_khb kh on b.ghsid=kh.khid
                             ...
                             select 1 xh,@chmc as chmc,@cfbl as cfbl,@ghsmc as ghsmc,@dj as dj  into #wz; 
                ...
                if (cxlx == "wgqr" || cxlx=="cjfl") {
                    ...
                    strResult.Append("ghsmc:'" + MyDt.Rows[0]["ghsmc"].ToString() + "',");
                    strResult.Append("dj:'" + MyDt.Rows[0]["dj"].ToString() + "',");
    ...
    public void getInfo( int userid,string scanctrl)
    {
        ...
        try
        {
            ...
            if (errInfo == "")
            {
                ...
                else if (scanctrl == "wgqr" && ryid.Rows[0]["wlkzx"].ToString() == "22" && ryid.Rows[0]["kzx"].ToString() == "20")// || scanctrl == "cjjs")裁剪接收不控制岗位
                {
                    CS = "false";
                } else {
                    CS = "ok";
