...
<script runat="server">
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (ctrl == "" || ctrl == null)
        {
            ...
            useridVal.Value = SystemKey;//系统管理->系统设置-微信部门人员管理(系统名称=协同系统,系统key就是t_user.id),不用控制权限,直接使用IM的菜单权限
    ...
    <script type="text/javascript">
        ...
        function goScan(result) {
            ...
            if (checkInfo.result == "Successed") {
                ...
                var scData = getGYD(result)
                createChdmListHtml(checkInfo, scData);//构造材料列表
                showChdmList();//显示材料列表                     
        ...
        //构造材料代码列表
        function createChdmListHtml(obj, scData) {
            $("#chxx").empty();
            var trBgClass;
            var rows = "";
            ...
            if (scData.WJpath.length > 0) {
                rows += "<a href='http://webt.lilang.com:9001/photo/yfspkh" + scData.WJpath + "' class='list-group-item list-group-item-success'>工艺单</a>";
            }
            if (scData.JYBGList.length > 0) {
                for (var i = 0; i < scData.JYBGList.length; i++) {
                    rows += "<a href='http://webt.lilang.com:9030/tl_sc/1/sc_ml_jy.aspx?djid=" + scData.JYBGList[i].Id + "&syid=" + scData.JYBGList[i].Syid + "&tzid=1&flowid=" + scData.JYBGList[i].Flowid + "' class='list-group-item list-group-item-success'>" + scData.JYBGList[i].Bgbh + "</a>";
                }
            }
            var checkInfos = obj.jsonsObject;
            var 材料与供货商信息 = checkInfos[0].材料与供货商信息;//
            rows += "<a href='http://webt.lilang.com:9030/tl_yf/yf_plk_wordkh.aspx?yphh=" + checkInfos[0].样号 + "' class='list-group-item list-group-item-success'>配料卡</a>";
        ...
        function bqLogSearch(obj) {
            ...
            var scData = getGYD(obj.innerHTML);
            createChdmListHtml(checkInfo, scData);//构造材料列表
            showChdmList();//显示材料列表
        }
