...
<script runat="server">
    protected void Page_Load(object sender, EventArgs e)
    {
            ...
            try
            {
                ...
                using (LiLanzDALForXLM dal = new LiLanzDALForXLM(OAConnStr))
                {
                    string str_sql = @"
                         ...
                         from  yx_t_ypdmb sp 
                         left join (select max(b.dj) as dj,b.yphh from yx_t_kcdjb a inner join yx_t_kcdjmx b on a.id=b.id where b.yphh=@bq and a.rq>='20170101' and a.djlx in (111,112,121,122,141,142,161,162,118,119,125,126,157,158) and a.shbs=1 and a.djbs=1 group by b.yphh) b on sp.yphh=b.yphh
                         where sp.yphh=@bq                   
                         ";
                    List<SqlParameter> para = new List<SqlParameter>();
        ...
        else if (ctrl == "saveInfo")
        {
            ...
            str_sql += " values ('1','','118','2407',1,@djh,getdate(),'11228',0,'12234','10660',0,0,0,0,@zdr,@zdr,@zdr,'',getdate(),getdate(),getdate(),1,1,0,'',0,'','',0,0,0,'','',0,'','1166')";
            ...
            for (int m = 0; m < info.Split(',').Length; m++)
            {
                ...
                str_sql += " insert into yx_t_kcdjmx  (id,yphh,shdm,sl,js,zxid,bj,zks,dj,je,cjje,zzsl,cbje,hscbje,rkjs,rksl,jysl,lymxid,djzt,sphh) values (@id,'" + sArray[2] + "','','" + sArray[1] + "',0,0,0,0,'" + sArray[3] + "',0,0,0,0,0,0,'" + sArray[1] + "',0,0,0,'" + sArray[0] + "');";
                str_sql += " set @mxid=SCOPE_IDENTITY()";
                str_sql += " insert into yx_t_kcdjcmmx(id,mxid,cmdm,sl0,rksl0) ";
                str_sql += " values(@id,@mxid,'cm24'," + sArray[1] + "," + sArray[1] + ");";
            ...
            str_sql += " select convert(varchar(6),a.rq,112)+a.djh+' ('+cast(b.sl as varchar)+')' as tsxx from yx_T_kcdjb a inner join (select sum(sl) as sl from yx_T_kcdjmx where id=@id) b on 1=1 where id=@id;";
            ...
            object djh;
            string errInfo = dal.ExecuteQueryFastSecurity(str_sql, para,out djh);
            Response.Clear();
            dal.Dispose();
            if (errInfo == "")
            {
                Response.Write("{result:'Successed',state:'ok',djh:'"+djh.ToString()+"'}");
            }
            else {
                Response.Write("{result:'Successed',state:'Fail',djh:''}" + errInfo);
            }
            Response.End();
        }
    }
        ...
        <div class="wrap-page">
            <div id="pagescan" class="page">
                <div class="input-group input-group-lg" style="margin-bottom: 5px;">
                    <span class="input-group-addon">上一单据号:</span>
                    <input type="text" class="form-control" id="befdjh" value="" readonly>
                </div>
